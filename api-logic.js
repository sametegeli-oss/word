...
function highlightEnglishWordssilinecek3(text) {
  if (!text) return '';
  ...
  // ADIM 5: TIRNAK İÇLERİNİ YEŞİL YAP
  // Düz çift tırnak "..." → <q>...</q>
  result = result.replace(/"([^"]+)"/g, '<q>$1</q>');
  // Düz tek tırnak '...' → <q>...</q>
  result = result.replace(/'([^']+)'/g, '<q>$1</q>');
  ...
  // ADIM 6: TÜM KELİMELERİ TIKLANABİLİR YAP
  const commonWords = ['the', 'and', 'are', 'for', 'was', 'with', 'you', 'that', 'this', 
                       'have', 'from', 'not', 'but', 'can', 'will', 'she', 'her', 'his', 
                       'him', 'they', 'them', 'their', 'had', 'has', 'been', 'were', 'said', 
                       'did', 'all', 'one', 'would', 'could', 'should', 'what', 'when', 
...
function highlightEnglishWordssilinecek2(text) {
  if (!text) return '';
  
  // SADECE TEMİZLİK YAP - HİÇ HIGHLIGHT YOK
  let result = text
    // AI'nın ürettiği HTML etiketlerini temizle
    .replace(/color:[^;>"]+["']?/gi, '')
    ...
    .replace(/style='[^']*'/gi, '')
    .replace(/<[^>]+>/g, '')
    // Markdown temizleme (agresif)
    .replace(/#{1,6}\s*/g, '')           // ### başlıklar
    .replace(/\*\*\*/g, '')              // *** bold+italic
    .replace(/\*\*/g, '')                // ** bold
    .replace(/\*/g, '')                  // * italic
    .replace(/_{1,2}/g, '')              // _ alt çizgi
    .replace(/~~(.+?)~~/g, '$1')         // ~~strikethrough~~
    .replace(/`{1,3}/g, '')              // ` code
    .replace(/---+/g, '')                // --- ayırıcı
    .replace(/^\s*[-*+]\s+/gm, '')       // - liste
    .replace(/^\s*\d+\.\s+/gm, '');      // 1. numaralı liste
  ...
  // Tırnak içi ifadeleri yeşil yap (tüm tırnak tipleri)
  result = result.replace(/"([^"]+)"/g, '<q>$1</q>');  // Düz çift tırnak
  result = result.replace(/'([^']+)'/g, '<q>$1</q>');  // Düz tek tırnak
  result = result.replace(/[""]([^""]+)["]/g, '<q>$1</q>');  // Smart quotes (curly)
  result = result.replace(/['']([^'']+)[']/g, '<q>$1</q>');  // Smart single quotes
  
  // Satır sonlarını <br> yap
  result = result.replace(/\n/g, '<br>');
  
  // DÜZ METİN - SADECE TIRNAKLAR YEŞİL
  return result;
}
...
function highlightEnglishWordsilinecek(text){
  // BU FONKSİYON KULLANILMIYOR AMA VARSA TEMİZLE
  if (!text) return '';
  ...
  // DÜZ METİN - HİÇBİR HIGHLIGHT YOK
  return result;
}
    ...
    ">
      ...
      <div style="display: flex; gap: 8px; margin-bottom: 12px; padding: 12px; background: var(--bg3); border-radius: 12px;">
        <button onclick="explanationNavigate(-1)" ${explanationHistoryIndex <= 0 ? 'disabled' : ''} style="
          ...
          opacity: ${explanationHistoryIndex <= 0 ? '0.5' : '1'};
        ">← Önceki</button>
        <div style="
          display: flex;
          align-items: center;
          ...
          color: var(--muted);
          white-space: nowrap;
        ">${explanationHistoryIndex + 1}/${explanationHistory.length}</div>
        <button onclick="explanationNavigate(1)" ${explanationHistoryIndex >= explanationHistory.length - 1 ? 'disabled' : ''} style="
          ...
          opacity: ${explanationHistoryIndex >= explanationHistory.length - 1 ? '0.5' : '1'};
        ">Sonraki →</button>
      </div>
      ` : ''}
      ...
      <div style="display: flex; flex-direction: column; gap: 8px;">
        ...
        <div style="
          ...
          font-weight: 800;
          text-align: center;
        ">✅ Bu kelime zaten listenizde</div>
        `}
...
async function renderLearn(){
  ...
  document.getElementById("wordCard").innerHTML=`
    ...
    ${sentHTML}
    <button onclick="showWordRelations('${item.word.replace(/'/g,"\\'")}','${(item.tr||'').replace(/'/g,"\\'")}','${(item.sentence||'').replace(/'/g,"\\'")}')" style="width:100%;padding:10px;margin:12px 0;background:linear-gradient(135deg,#8b5cf6,#7c3aed);border:none;border-radius:10px;color:#fff;font-size:13px;font-weight:800;cursor:pointer;font-family:'Nunito',sans-serif;transition:all 0.2s" onmouseenter="this.style.transform='translateY(-2px)'" onmouseleave="this.style.transform='translateY(0)'">
      🔗 Kelime İlişkileri
    </button>
...
function updateVisibleItems(){
  ...
  for(let i = visibleStart; i < visibleEnd; i++){
    ...
    // Filtrelenmiş listede mi yoksa tam listede mi kontrol et
    itemDiv.onclick = () => goToWord(i, wordList);
    itemDiv.innerHTML = `
...
function highlightEnglishWords(text) {
  ...
  // ADIM 2: Markdown kalıntılarını temizle (agresif)
  result = result
    .replace(/#{1,6}\s*/g, '')           // ### başlıklar
    .replace(/\*\*\*/g, '')              // *** bold+italic
    .replace(/\*\*/g, '')                // ** bold
  ...
  // ADIM 5: İngilizce kelimeleri vurgula (sadece harf içerenler)
  ...
  // ADIM 6: Tırnak içi ifadeleri yeşil yap (tüm tırnak tipleri)
  result = result.replace(/&quot;([^&]+)&quot;/g, '<q>$1</q>');  // HTML escaped "
  result = result.replace(/&#39;([^&#]+)&#39;/g, '<q>$1</q>');  // HTML escaped '
  result = result.replace(/[""]([^""]+)["]/g, '<q>$1</q>');  // Smart quotes
  result = result.replace(/['']([^'']+)[']/g, '<q>$1</q>');  // Smart single quotes
  result = result.replace(/„([^"]+)"/g, '<q>$1</q>');  // German quotes
...
function showToLearnList() {
  ...
  list.innerHTML = toLearn.map(item => `
    <div style="display:flex;align-items:center;gap:8px;padding:8px;background:var(--bg3);border-radius:8px">
      ...
      <button onclick="removeToLearnWord('${item.key}')" style="padding:4px 8px;background:var(--red);color:#fff;border:none;border-radius:6px;cursor:pointer;font-size:11px">Sil</button>
    </div>
  `).join('');
}
...
async function showWordRelations(word, tr, sentence) {
  ...
  try {
    const prompt = `Analyze the English word "${word}" (Turkish: ${tr}).

Provide:
1. **Synonyms** (eş anlamlı): 5 words with Turkish meanings
2. **Antonyms** (zıt anlamlı): 3-5 words with Turkish meanings
3. **Related words** (ilgili kelimeler): 5 words in same topic/context with Turkish meanings
4. **Collocations** (kelime öbekleri): 3-5 common phrases with this word
...
- phrase1 (türkçe)`;
    ...
    const response = await callAI('You are an English vocabulary expert.', prompt, 'explain');
    
    // Sonuçları göster
    const resultDiv = content.querySelector('div[style*="padding:20px"]');
    resultDiv.innerHTML = `
      <div style="font-size:15px;line-height:1.9;color:var(--text);white-space:pre-wrap">${response.content}</div>
...
function makeRelatedWordsClickable(container) {
  // İngilizce kelimeleri bul ve tıklanabilir yap
  const text = container.innerHTML;
  const wordPattern = /- ([a-zA-Z]+)/g;
  
  container.innerHTML = text.replace(wordPattern, (match, word) => {
    return `- <span style="color:var(--blue);cursor:pointer;text-decoration:underline" onclick="explainWord('${word}','wordCard');document.querySelector('.modal-overlay')?.remove()">${word}</span>`;
  });
}
