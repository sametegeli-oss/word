...
function highlightEnglishWords(text) {
  ...
  if (highlightSettings && highlightSettings.quotes?.enabled) {
    result = result.replace(/"([^"\n]{1,200})"/g, (fullMatch, content) => {
      const highlighted = content.replace(/\b([A-Za-z]{2,})\b/g, (word) => {
        return `<span class="ai-word highlight clickable-word" data-word="${word.toLowerCase()}">${word}</span>`;
      });
      return `<span class="ai-word warning">"${highlighted}"</span>`;
    });
  }
  
  // ADIM 4: Parantez içi
  if (highlightSettings && highlightSettings.parens?.enabled) {
    result = result.replace(/\(([^)<>\n]{1,60})\)/g, `<span class="ai-word accent">($1)</span>`);
  }
  ...
  result = parts.map((part) => {
    ...
    processed = processed.replace(/\b([A-Z][a-z]{2,}|[a-z]{4,})\b/g, (word) => {
      ...
      return `<span class="ai-word success clickable-word" data-word="${word.toLowerCase()}">${word}</span>`;
    });
