...
async function checkGroqUsage(){
  ...
  if(!el) return;
  if(!GROQ_API_KEY){ el.innerHTML = '<span style="color:var(--red)">❌ API key girilmemiş</span>'; return; }
...
const BADGES = {
  ...
  listener:      { id: 'listener',      name: 'Dinleyici',         desc: '5 dinleme testi tamamla',   icon: '👂', requirement: 5,    cat:'oyun' },
...
async function openBook(bookId){
  ...
  // Önce localStorage'dan bak
  const cached = localStorage.getItem('book_text_'+bookId);
  let text = cached;
  ...
  if(!text || text.length < 500){
    ...
    try {
      ...
      text = ch1;
      // İlk bölümü cache'e kaydet (aiGenerated flag ile)
      localStorage.setItem('book_text_'+bookId, text);
      localStorage.setItem('book_meta_'+bookId, JSON.stringify({title:book.title, author:book.author, level:book.level, aiGenerated:true, chapterNum:1}));
    } catch(e){
      document.getElementById('readerContent').innerHTML =
    ...
    // Cache'e kaydet (max 200KB)
    if(text.length < 200000){
      localStorage.setItem('book_text_'+bookId, text);
      localStorage.setItem('book_meta_'+bookId, JSON.stringify({title:book.title, author:book.author, level:book.level}));
    }
  }
...
function makeReaderWordsClickable(html){
  try {
    return html.replace(/\b([A-Za-z]{3,})\b/g, (match) => {
      ...
      const found = allWords && allWords.find(w => w.word.toLowerCase() === match.toLowerCase());
      const cls = found ? 'clickable-word known-word' : 'clickable-word';
      return '<span class="'+cls+'" onclick="explainWord(\''+match+'\',\'readerContent\')">'+match+'</span>';
    });
  } catch(e){ return html; }
}
  ...
  if(readerChunkIdx < readerChunks.length-1){
    ...
    try {
      ...
      if(newChapter && newChapter !== '__RATE_LIMIT__' && newChapter.length > 100){
        ...
        // Cache'e ekle
        const existingText = localStorage.getItem('book_text_'+currentBook?.id) || '';
        localStorage.setItem('book_text_'+currentBook?.id, existingText+'\n\n'+newChapter);
...
function highlightEnglishWordsilinecek(text){
  ...
  let result = text
    ...
    .replace(/<br\s*\/?>/gi, '\n')
    .replace(/&lt;br\s*\/?&gt;/gi, '\n')
    // HTML/CSS inline kodları (AI bazen bunları metin olarak üretiyor)
    .replace(/<\/?[a-z]+[^>]*>/gi, '')  // <div>, <span style="...">, </div> vb.
    .replace(/&lt;\/?[a-z]+[^&]*&gt;/gi, '')  // Escape edilmiş HTML tagları
  ...
  // ADIM 2: Markdown formatting
  // **bold**
  result = result.replace(/\*\*([^*\n]+?)\*\*/g, '<span style="color:#60a5fa;font-weight:800">$1</span>');
  // *italic*
  result = result.replace(/\*([^*\n]+?)\*/g, '<span style="color:#a78bfa;font-weight:700">$1</span>');
  // `code`
  result = result.replace(/`([^`\n]+?)`/g, '<code style="color:#4ade80;font-family:monospace">$1</code>');

  // ADIM 3: Tırnak içindeki kelimeleri işaretle (escape edilmiş " yok artık, & var)
  // Tırnakları bul ve içeriği işaretle
  result = result.replace(/"([^"\n]{1,200})"/g, (fullMatch, content) => {
    // İçindeki İngilizce kelimeleri clickable yap (kırmızı)
    const highlighted = content.replace(/\b([A-Za-z]{2,})\b/g, (word) => {
      return `<span class="en-word clickable-word" data-word="${word.toLowerCase()}" style="color:#ef4444;font-weight:700;cursor:pointer">${word}</span>`;
    });
    return `<span style="color:${highlightSettings.quotes.color};font-weight:700">"${highlighted}"</span>`;
  });

  // ADIM 4: Parantez içi
  if(highlightSettings.parens.enabled){
    result = result.replace(/\(([^)<>\n]{1,60})\)/g, `<span style="color:${highlightSettings.parens.color};font-weight:700">($1)</span>`);
  }

  // ADIM 5: Tırnak DIŞINDA kalan İngilizce kelimeleri işaretle (yeşil)
  const parts = result.split(/(<[^>]+>)/g);
  result = parts.map((part) => {
    ...
    // Tırnak dışındaki İngilizce kelimeleri yeşil yap
    processed = processed.replace(/\b([A-Z][a-z]{2,}|[a-z]{4,})\b/g, (word) => {
      // Türkçe karakter içeriyorsa atla
      if(/[çğıöşüÇĞİÖŞÜ]/.test(word)) return word;
      return `<span class="en-word clickable-word" data-word="${word.toLowerCase()}" style="color:#22c55e;font-weight:600;cursor:pointer">${word}</span>`;
    });
...
async function explainWord(word, containerId) {
  // Loading göster
...
Keep it concise and beginner-friendly.`;
  ...
  try {
    // Dinamik AI çağrısı - gerçek kullanılan modeli döndürür
    const response = await callAI('You are an English teacher explaining words to Turkish learners.', prompt, 'explain');
    ...
    console.log('📊 Kullanılan Model:', response.model);
    console.log('🔤 Token Limiti:', response.tokenLimit);
    
    // Modal'da göster - GERÇEK model bilgisiyle
...
function showWordExplanationModal(word, explanation, containerId, aiType = 'explain', actualModel = null, actualTokenLimit = null) {
  // Gerçek kullanılan model bilgisini al - eğer verilmemişse ayarlardan al
  const usedModel = actualModel || getAIModel(aiType);
  const usedTokenLimit = actualTokenLimit || getAITokenLimit(aiType);
  
  console.log('🎨 Modal açılıyor - Model:', usedModel, 'Token:', usedTokenLimit);
  ...
  modal.innerHTML = `
    ...
    ">
      ...
      ">${highlightEnglishWords(explanation)}</div>
      ...
      <button onclick="speak('${word.replace(/'/g, "\\'")}', 'en-US')" style="
        ...
        font-weight: 800;
        cursor: pointer;
      ">🔊 Telaffuz Dinle</button>
    </div>
  `;
  
  document.body.appendChild(modal);
  
  // İngilizce kelimelere tıklanabilirlik ekle - DİREKT BULMA
  setTimeout(() => {
    // Tüm clickable-word'leri direkt document'ten bul
    const allClickableWords = document.querySelectorAll('#wordExplanationModal span.clickable-word');
    
    console.log('🔍 Bulunan clickable-word sayısı:', allClickableWords.length);
    
    if(allClickableWords.length === 0) {
      console.warn('⚠️ Hiç clickable-word bulunamadı!');
      return;
    }
    
    allClickableWords.forEach((span, index) => {
      // Stil ayarları
      ...
      span.onclick = async function(e) {
        ...
        console.log('🔍 Kelimeye tıklandı:', clickedWord);
        
        // Modal'ı kapat
        closeWordExplanation();
        
        // Küçük delay (modal kapanması için)
        await new Promise(resolve => setTimeout(resolve, 100));
        
        // Yeni kelimeyi açıkla
        await explainWord(clickedWord, containerId);
      };
    });
    
    console.log('✅ Tüm kelimelere click event eklendi!');
    
  }, 300); // Biraz daha bekle
...
function showErr(msg){const b=document.getElementById("errBox");b.textContent="⚠️ "+msg;b.style.display="block";}
...
function loadFile(file){
  ...
  reader.onload=e=>{
    try{
      ...
      const colSentTr = headers["sentencetr"] ?? headers["sentenceTr"] ?? 
                        headers["cümleçeviri"] ?? null;
...
function saveProgress(){
  try{
    localStorage.setItem(fileKey,JSON.stringify({learnedWords:[...learnedSet],wordStatus,score,streak,correctCount,idx}));
    localStorage.setItem(fileKey+"_words",JSON.stringify(allWords));
    localStorage.setItem("wm_lastKey",fileKey);
    saveTimingData();
  }catch(e){
    console.error('❌ Progress save error:', e);
    // Quota exceeded hatası kontrolü
...
function mkSentColored(sentence,highlights,colors){
  ...
  return sentence.split(/(\s+)/).map(p=>{
    ...
    if(col) return `<span style="color:${col};font-weight:800">${p}</span>`;
    if(highlights&&highlights.some(h=>h&&h.toLowerCase()===c)) return `<span class="hl">${p}</span>`;
    return p;
  }).join("");
}
...
// NAVIGATION
// ══════════════════════════════════════════════════════════
function prevWord(){if(idx>0){idx--;phase="learn";renderLearn();}}
...
function updateVisibleItems(){
  ...
  for(let i = visibleStart; i < visibleEnd; i++){
    ...
    const sentPreview = w.sentence ? `<div style="font-size:13px;color:var(--muted);margin-top:4px;line-height:1.4">${w.sentence.length > 80 ? w.sentence.substring(0, 80) + '...' : w.sentence}</div>` : "";
    ...
    itemDiv.onclick = () => goToWord(i);
    itemDiv.innerHTML = `
      <div class="wi-ico">${ico}</div>
      <div class="wi-body">
        <div class="wi-word">${w.word}${rowBadge}</div>
        <div class="wi-tr">${w.tr}${w.phonetic ? ' · <span style="color:var(--purple)">' + w.phonetic + '</span>' : ''}</div>
        ${sentPreview}
      </div>
...
function goToWord(allWordsIndex){
  // Direkt allWords index'ine git
  // words array'i öğrenilmemiş kelimeleri filtreler, o yüzden karışıyor
  // Çözüm: idx'i allWords index'i yap, ama words'ü rebuild et
  
  // Önce tüm kelimeleri göster
  words = [...allWords];
  idx = allWordsIndex;
  phase = "learn";
  showScreen("sc-word");
...
function speak(text,lang){
  ...
  console.log('🎤 Sesler:', voices.map(v => v.name + ' (' + v.lang + ')').join(', '));
...
function speakPartner(text){
  // Partner (Sarah) için kadın sesi seç
  ...
  u.pitch = 1.1;
  const voices = speechSynthesis.getVoices();
  // Kadın sesi öncelikleri
  const femaleKeywords = ['female','woman','girl','zira','samantha','victoria','karen','moira','fiona','tessa','veena'];
  let voice = voices.find(v => v.lang.startsWith('en') && femaleKeywords.some(k => v.name.toLowerCase().includes(k)));
  // Bulunamazsa en-US'daki ikinci sesi dene (genellikle kadın)
  if(!voice){
    const enVoices = voices.filter(v => v.lang.startsWith('en'));
    voice = enVoices[1] || enVoices[0];
  }
...
async function generateStory(){
  ...
  // Öğrenilen kelimeleri al (en az 5, en fazla 15)
  const learnedWords = [...learnedSet].slice(0, 15);
  
  if(learnedWords.length < 3){
    document.getElementById("storyContent").innerHTML = `
      <div style="text-align:center;padding:40px 20px">
        <div style="font-size:48px;margin-bottom:12px">😅</div>
        <p style="color:var(--muted)">En az 3 kelime öğrenmelisin!</p>
        <p style="font-size:13px;color:var(--muted);margin-top:8px">Öğrendikten sonra hikaye oluşturabilirim.</p>
      </div>`;
  ...
  document.getElementById("storyContent").innerHTML = `
    <div style="text-align:center;padding:40px 20px;color:var(--muted)">
      <div style="font-size:48px;margin-bottom:12px">✨</div>
      <p>Şu kelimelerle hikaye yazıyorum:</p>
      <div style="margin-top:12px;font-weight:700;color:var(--text)">${learnedWords.join(", ")}</div>
    </div>`;
  
  const prompt = `Write a short, fun and engaging story in ENGLISH (150-200 words) using the following words:

Words: ${learnedWords.join(", ")}

RULES:
1. The story must be entirely in ENGLISH
2. Use ALL the given words in the story
3. Make the target words **bold** (e.g. **collaborate**)
4. The story should be fluent, natural and interesting
5. At the very end, add a short Turkish summary (2-3 sentences) under the label "📝 Türkçe Özet:"

Start the story immediately, no intro!`;
  ...
  try {
    ...
    const aiResponse = await callAI(
      "You are a creative story writer. You write short English stories for language learners.",
      prompt,
    ...
    document.getElementById("storyContent").innerHTML = `
      <div style="background:var(--bg2);padding:16px;border-radius:12px;margin-bottom:12px" id="storyTextContainer">
        <div style="font-weight:700;color:var(--purple);margin-bottom:8px;font-size:12px">📚 Hikaye (${learnedWords.length} kelime)</div>
    ...
    if (enableWordClick) {
      ...
      if (storyContainer) {
        storyContainer.addEventListener('dblclick', handleWordDoubleClick);
...
function initPodcastList() {
  ...
  podcastDatabase.forEach(podcast => {
    const card = document.createElement('div');
    card.style.cssText = 'background:var(--bg2);padding:14px;border-radius:12px;border:1px solid var(--border);cursor:pointer;transition:all .2s';
    card.onmouseover = () => card.style.background = 'var(--bg3)';
    card.onmouseout = () => card.style.background = 'var(--bg2)';
    card.onclick = () => openPodcast(podcast.id);
    
    card.innerHTML = `
      <div style="display:flex;justify-content:space-between;align-items:start;margin-bottom:8px">
        <div style="font-weight:800;font-size:14px;color:var(--text)">${podcast.title}</div>
        <div style="padding:4px 8px;background:var(--purple);color:#fff;border-radius:6px;font-size:10px;font-weight:700">${podcast.level}</div>
      </div>
      <div style="font-size:11px;color:var(--muted);margin-bottom:6px">🎙️ ${podcast.source} • ⏱️ ${podcast.duration}</div>
      <div style="display:flex;gap:4px;flex-wrap:wrap">
        ${podcast.keywords.slice(0, 3).map(k => `<span style="padding:3px 8px;background:var(--bg3);border-radius:6px;font-size:10px;color:var(--sub)">${k}</span>`).join('')}
      </div>
    `;
...
function openPodcast(podcastId) {
  currentPodcast = podcastDatabase.find(p => p.id === podcastId);
  if (!currentPodcast) return;
  
  document.getElementById('podcastTitle').textContent = currentPodcast.title;
  
  // Transcript'i göster - satır atlamaları için <br> ekle
  const transcriptEl = document.getElementById('podcastTranscript');
  transcriptEl.innerHTML = currentPodcast.transcript.replace(/\n/g, '<br>');
  
  // Audio player (şu an için devre dışı - gerçek URL'ler eklenince açılacak)
  const audioEl = document.getElementById('podcastAudio');
  if (currentPodcast.audioUrl && currentPodcast.audioUrl !== '#') {
    audioEl.src = currentPodcast.audioUrl;
  ...
  document.getElementById('extractedWordsSection').style.display = 'none';
  
  // Transcript'e kelime tıklama ekle
  if (enableWordClick) {
    transcriptEl.addEventListener('dblclick', handleWordDoubleClick);
    transcriptEl.addEventListener('touchend', handleMobileTouchEnd);
  }
  
  showToast('🎧 Podcast Yüklendi', `${currentPodcast.title} hazır!`);
}
...
function playRec(){
  ...
  audioPlayer.play();
  audioPlayer.onended=()=>{if(btn) btn.textContent="▶ Kaydı Dinle";};
}

// ── Harf bazlı renkli karşılaştırma ──
function colorLetters(target, heard) {
  // Kelime bazlı karşılaştır, her kelimenin harflerini renklendir
  const tWords = target.split(/\s+/);
  const hWords = heard.split(/\s+/);

  return tWords.map((tw, wi) => {
    const hw = hWords[wi] || '';
    const dist = levDist(tw, hw);

    // Kelime tamamen doğruysa tümü yeşil
    if (tw === hw) {
      return `<span style="display:inline-block;margin:2px">${tw.split('').map(l =>
    ...
    let html = `<span style="display:inline-block;margin:2px">`;
    for (let i = 0; i < tw.length; i++) {
      ...
      if (tl === hl) {
        ...
        html += `<span style="color:#f87171;font-weight:800;font-size:17px;text-decoration:underline">${tl}</span>`;
      } else {
        html += `<span style="color:#f87171;opacity:.5;font-weight:800;font-size:17px">${tl}</span>`;
      }
    }
    html += `</span>`;
    return html;
    ...
    if(lt){
      lt.innerHTML=`
        ...
        <div style="background:#1a0a2e;border:1px solid var(--purple);border-radius:10px;padding:10px;margin-bottom:8px;display:flex;align-items:center;gap:8px">
          ...
          <button onclick="speakTurkish('${trText.replace(/'/g,"\\'")}')" style="padding:6px 10px;background:#4c1d95;border:none;border-radius:8px;font-family:'Nunito',sans-serif;font-weight:700;font-size:12px;cursor:pointer;color:#c4b5fd;flex-shrink:0">🔊 Dinle</button>
        </div>` : ''}
        ...
        <div style="display:flex;gap:8px">
          ...
          <button onclick="playRec()" style="flex:1;padding:8px;background:#1e3a5f;border:none;border-radius:10px;font-family:'Nunito',sans-serif;font-weight:700;font-size:13px;cursor:pointer;color:#93c5fd">▶ Kaydımı Dinle</button>
  ...
  for(const q of[kw[0],kw.slice(0,2).join(" "),kw.join(" ")]){
    try{
      ...
      if(page&&page.thumbnail?.source){
        ...
        img.onload=()=>{wrap.style.display="block";credit.innerHTML="📷 Wikipedia";};
...
function switchTab(tab){
  ...
  else if(tab==="settings"){showScreen("sc-settings");loadAITokenSettings();updateLastSaveTime();}
}
...
function smLongPressStart(i){
  ...
  lpTimer=setTimeout(()=>{
    ...
    explainWord(clean,'chatMessages');
  },600);
}
...
Öğretici ve motive edici ol!`;
  ...
  document.getElementById("chatMessages").innerHTML=`<div class="chat-msg ai">${highlightEnglishWords(response).replace(/\n/g,"<br>")}</div>`;
...
function openPronCoach(){
  ...
  document.getElementById("coachStatus").innerHTML=`
    <div style="text-align:center;padding:20px">
      <div style="font-size:48px;margin-bottom:12px">🎤</div>
      <div style="font-size:24px;font-weight:800;color:var(--green);margin-bottom:8px">${word.word}</div>
      <p style="color:var(--muted)">Bu kelimeyi seslendir, AI analiz etsin</p>
      <!-- Dalga Grafiği -->
...
Keep it short and encouraging.`;
    ...
    try {
      // DİNAMİK AI SİSTEMİ
      const aiResponse = await callAI("You are a pronunciation coach.", prompt, 'pronun');
...
function recordVoiceSample(){
  ...
  btn.textContent="🔴 Kaydediyor...";
  btn.disabled=true;
  
  document.getElementById("voiceCloneStatus").innerHTML=`
    <div class="correction-box" style="margin-top:16px">
      <div style="text-align:center;padding:20px">
        <div style="font-size:48px;margin-bottom:12px">🎙️</div>
        <div style="font-size:18px;font-weight:800;color:var(--red);margin-bottom:8px">Kaydediliyor...</div>
        <p style="color:var(--muted)">Şunu oku: "Hello, I am learning English with this app."</p>
      </div>
    </div>`;
  ...
  setTimeout(()=>{
    ...
    voiceCloned=true;
    document.getElementById("cloneStep2").style.opacity="1";
    document.getElementById("btnTestClone").disabled=false;
    
    document.getElementById("voiceCloneStatus").innerHTML=`
      <div class="correction-box" style="margin-top:16px;border-color:var(--green)">
        <div style="text-align:center;padding:20px">
          <div style="font-size:48px;margin-bottom:12px">✅</div>
          <div style="font-size:18px;font-weight:800;color:var(--green)">Sesin Klonlandı!</div>
          <p style="color:var(--muted);margin-top:8px">Artık kelimeler senin sesinle okunacak. Word Mode'dan herhangi bir kelimeyi aç ve 🔊 butonuna bas.</p>
        </div>
      </div>`;
  },5000);
}
...
function testClonedVoice(){
  if(!voiceCloned) return;
  alert("✅ Ses klonlama aktif!\nWord Mode'da kelimeleri dinlediğinde senin sesin kullanılacak.\n\nNOT: Bu özellik simülasyondur - gerçek ses klonlama için 11ty.ai veya ElevenLabs API gerekir.");
}
...
async function analyzeVideo(){
  ...
  if(!url){alert("YouTube linki gir");return;}
  
  document.getElementById("videoAnalysisResult").innerHTML=`
    <div class="correction-box">
      <div style="text-align:center;padding:30px">
        <div style="font-size:48px;margin-bottom:12px">🎬</div>
        <p style="color:var(--muted)">Video analiz ediliyor...</p>
        <p style="font-size:12px;color:var(--muted);margin-top:8px">(Gerçek YouTube API entegrasyonu gerekiyor)</p>
      </div>
    </div>`;
  
  setTimeout(async ()=>{
    const prompt=`Imagine this YouTube video is about "Technology and Future". Extract 15 important English words from it with Turkish translations. Format:
word - türkçe
word - türkçe
...`;
    
    const wordList=await callGroqAPI("You extract words from video transcripts.",prompt);
    
    document.getElementById("videoAnalysisResult").innerHTML=`
      <div class="correction-box">
        <div class="correction-label">📋 Videodan Çıkarılan Kelimeler</div>
        <div class="correction-explain" style="white-space:pre-line">${wordList}</div>
      </div>
      <button class="btn btn-green" onclick="exportVideoWords()">📥 Kelimeleri Excel'e Aktar</button>
      <p style="font-size:11px;color:var(--muted);margin-top:8px;text-align:center">
        NOT: Gerçek entegrasyon için YouTube Transcript API gerekiyor
      </p>`;
  },2000);
}

function exportVideoWords(){
  alert("📥 Excel export özelliği yakında!\nŞimdilik kelimeleri kopyalayıp manuel olarak Excel'e yapıştırabilirsin.");
}
...
async function sendContextAnalysis() {
  ...
  const prompt = `"${text}" için bağlam analizi yap:
1. Farklı anlamları
2. Her anlam için örnek cümle (İngilizce - sadece bu tırnak içinde)
3. Türkçe çeviri (tırnak KULLANMA - parantez içinde yaz)
4. Hangi durumlarda kullanılır

Format:
1. [Anlam açıklaması]
   Örnek: "İngilizce örnek cümle"
   (Türkçe çeviri tırnak kullanmadan)

Kısa ve öz ol, en fazla 3 farklı bağlam göster.`;
  ...
  try {
    ...
    const aiResponse = await callAI(
      "Sen bir İngilizce öğretmenisin. Kelimelerin ve cümlelerin bağlamını analiz edersin.",
      prompt,
...
If no REAL grammar errors, say: "✅ Cümle doğru!"`;

  const response=await callGroqAPI("Sen bir İngilizce öğretmenisin. Cümleleri düzeltir ve öğretirsin.",prompt);
  ...
  if (enableWordClick) {
    ...
    if (correctionBox) {
      correctionBox.addEventListener('dblclick', handleWordDoubleClick);
...
function renderFlashcard(){
  ...
  document.getElementById("fcPhonetic").textContent=item.phonetic||"";
  // Türkçe tarafta: kelime çevirisi + cümle (sentenceTr veya sentence)
  document.getElementById("fcTranslation").textContent=item.tr||item.word;
  document.getElementById("fcSentence").textContent=item.sentence||"";
  document.getElementById("fcSentenceTr").textContent=item.sentenceTr||item.sentence||"";
...
async function callGroqAPI(systemPrompt, userMessage, maxTokens) {
  ...
  // API Key kontrolü - önce apiKeys objesinden, sonra global GROQ_API_KEY'den
  const apiKey = apiKeys.groq || GROQ_API_KEY;
  ...
  try {
    ...
    if(!response.ok){
      ...
      if(response.status===429) return '__RATE_LIMIT__';
      return null;
    }
...
function handleWordDoubleClick(event) {
  // Çift tıklama otomatik olarak kelimeyi seçer
  const selection = window.getSelection();
  let word = selection.toString().trim();
  ...
  console.log('📖 Word double-clicked:', word);
  explainWord(word, 'chatMessages');
...
- **bold** için çift yıldız kullan
- *italic* için tek yıldız kullan
- Örnekleri "tırnak" içinde göster`;
    fullQuestion = msg;
  } else {
...
Madde işaretleri kullan, örnekler ver. Cümlenin dilbilgisi yapısını, zamanını ve kelime görevlerini açıkla.`;
  ...
  try {
    ...
    // ══════════════════════════════════════════════════════════
// KESİN ÇÖZÜM - Tüm AI HTML/CSS kalıntılarını temizler
// ══════════════════════════════════════════════════════════

function highlightEnglishWords(text) {
  if (!text) return '';
  
  // ADIM 1: Tüm CSS/inline style kalıntılarını temizle (EN ÖNCE)
  let result = text
    // color:#xxx ve font-weight:xxx kalıpları
    ...
    // Tırnak içindeki stil blokları
    /css
    .replace(/style="[^"]*"/gi, '')
    .replace(/style='[^']*'/gi, '')
    // Tekli CSS kuralları (color:#fff gibi)
    .replace(/\bcolor:\s*#[a-fA-F0-9]{3,6}\b/gi, '')
    .replace(/\bfont-weight:\s*\d{1,3}\b/gi, '')
    // Artık HTML etiketleri
    .replace(/<color[^>]*>/gi, '')
    .replace(/<\/color>/gi, '')
    .replace(/<span[^>]*>/gi, '')
    .replace(/<\/span>/gi, '')
    ...
    .replace(/<em[^>]*>/gi, '*')
    .replace(/<\/em>/gi, '*')
    // Geriye kalan tüm HTML tagları
    .replace(/<[^>]+>/g, '');
  
  // ADIM 2: Markdown kalıntılarını düzenle
  result = result
    .replace(/\*\*\s*\*\*/g, '') // boş bold
    .replace(/\*\s+\*/g, '')     // boş italic
    .replace(/&lt;/g, '<')
    .replace(/&gt;/g, '>')
    .replace(/&amp;/g, '&');
  
  // ADIM 3: Fazla boşlukları temizle
  result = result.replace(/\s+/g, ' ').trim();
  
  // ADIM 4: HTML karakterlerini escape et (güvenlik)
  result = result
    .replace(/&/g, '&amp;')
    .replace(/</g, '&lt;')
    .replace(/>/g, '&gt;');
  
  // ADIM 5: Güvenli markdown işleme
  result = result.replace(/\*\*([^*\n]+?)\*\*/g, '<strong style="color:#60a5fa">$1</strong>');
  result = result.replace(/\*([^*\n]+?)\*/g, '<em style="color:#a78bfa">$1</em>');
  
  // ADIM 6: İngilizce kelimeleri vurgula (sadece harf içerenler)
  const words = result.split(/(\s+)/);
  result = words.map(word => {
    // Sadece harflerden oluşan ve 3+ karakterli kelimeler
    if (/^[A-Za-z]{3,}$/.test(word)) {
      // Türkçe kelimeleri atla
      const turkishStop = ['ve', 'ile', 'ki', 'de', 'da', 'bir', 'bu', 'şu', 'o', 'ben', 'sen', 'onlar', 'biz', 'siz'];
      if (turkishStop.includes(word.toLowerCase())) return word;
...
function cleanAIResponse(rawResponse) {
  ...
  // Markdown bold/italic'i düzgün hale getir
  cleaned = cleaned
    .replace(/\*\*([^*]+?)\*\*/g, '**$1**')
    .replace(/\*([^*]+?)\*/g, '*$1*');
  ...
  return cleaned;
    ...
    const highlighted = highlightEnglishWordssilinecek2(response);
    typingDiv.innerHTML = highlighted.replace(/\n/g, '<br>');
...
Seviye: ${aiUserLevel}
${levelPrompts[aiUserLevel]}
Görevin: Öğrencinin İngilizce sorusunu detaylı, eğitici ve teşvik edici şekilde cevapla. Madde işaretleri kullan, örnekler ver. Cümlenin dilbilgisi yapısını, zamanını ve kelime görevlerini açıkla.`;
  ...
  try {
    // DİNAMİK AI SİSTEMİ
    const aiResponse = await callAI(systemPrompt, fullQuestion, 'chat');
    let response = String(aiResponse.content || aiResponse); // String'e çevir
    ...
    // Model badge ekle
    if(aiResponse.model) {
      const usedModel = aiResponse.model;
      const usedTokenLimit = aiResponse.tokenLimit;
      ...
      badge.style.cssText = `
        ...
        margin-left: 8px;
      `;
      badge.innerHTML = `🤖 ${modelName} <span style="opacity:0.6">• ${usedTokenLimit} token</span>`;
      
      aiWrapper.appendChild(badge);
      
      console.log('💭 Chat - Kullanılan Model:', usedModel, 'Token:', usedTokenLimit);
    }
  ...
  aiStopBtn.onclick = () => {
    if (window.speechSynthesis) {
      window.speechSynthesis.cancel();
      console.log('🔇 AI Chat sesi durduruldu');
    }
  };
  buttonRow.appendChild(aiStopBtn);
...
function addPartnerMsg(text, role) {
  ...
  div.innerHTML = (role === 'ai' ? highlightEnglishWords(text) : text).replace(/\n/g, '<br>');
...
async function sendPartnerMsg() {
  ...
  // Kelime tıklama ekle
  if (enableWordClick) {
    typingEl.addEventListener('dblclick', handleWordDoubleClick);
...
function loadShadowPhrases() {
  ...
  shadowPhrases = available.slice(0, 20).map(w => ({
    en: w.sentence,
    tr: w.sentenceTr || '',
    word: w.word,
    done: false,
...
function selectShadowPhrase(i) {
  ...
  document.getElementById('shadowTargetText').textContent = phrase.en;
...
function playShadowAudio() {
  ...
  setTimeout(() => {
    ...
    utt.onend = () => {
      ...
      if (playBtn) playBtn.textContent = '▶ Dinle';
    };
...
const PERSONAS = {
  strict: { emoji:'👩‍🏫', prompt:'You are a strict English teacher. Correct every grammar mistake immediately and explicitly. Be thorough but not unkind.' },
  friendly: { emoji:'🤗', prompt:'You are a warm and friendly English teacher. Focus on encouragement. Correct mistakes gently at the end of your response.' },
  socratic: { emoji:'🧐', prompt:'You are a Socratic English teacher. Instead of giving answers, ask questions to guide the student to discover the answer themselves.' },
  coach: { emoji:'⚡', prompt:'You are a motivational English coach. High energy, goal-focused. Push the student to practice more and celebrate every improvement.' }
};
...
Respond in Turkish for explanations, use English for examples. Keep responses focused and practical.`;
  ...
  typingDiv.innerHTML = highlightEnglishWords(cleanResponse).replace(/\n/g, '<br>');
...
function setPodcastLevel(lvl){
  podcastLevel = lvl;
  document.querySelectorAll('[id^="plvl-"]').forEach(b=>b.classList.remove('active'));
  document.getElementById('plvl-'+lvl)?.classList.add('active');
  PODCAST_EPISODES = []; // Reset — yeni seviyede üret
  generatePodcastEpisodes();
}

async function generatePodcastEpisodes(){
  if(podcastGenerating) return;
  podcastGenerating = true;

  const list = document.getElementById('podcastList');
  list.innerHTML = '<div style="text-align:center;padding:24px;color:var(--muted)"><div style="font-size:28px">🤖</div><div style="margin-top:8px;font-size:13px">AI podcast içerikleri hazırlanıyor...</div></div>';

  // 4 rastgele konu seç
  const shuffled = [...PODCAST_TOPICS].sort(()=>Math.random()-.5).slice(0,4);
  PODCAST_EPISODES = [];
  ...
  for(let i=0; i<shuffled.length; i++){
    ...
    try {
      // Dinamik AI çağrısı
      const selectedModel = document.getElementById('podcastAIModel')?.value || 'groq';
      console.log('🎧 Podcast AI Model:', selectedModel);
      
      const aiResponse = await callAI(
        'You are a podcast script writer. Write natural, conversational English.',
        `Write a short podcast transcript (150-200 words) about: ${t.topic}
        
Level: ${podcastLevel} (${podcastLevel<='A2'?'beginner':podcastLevel<='B2'?'intermediate':'advanced'})
Style: Natural spoken English, like a real podcast.
${t.level === 'Beginner' ? 'Use simple vocabulary and short sentences.' : ''}
${t.level === 'Advanced' ? 'Use sophisticated vocabulary and complex ideas.' : ''}

Start directly with the content. No titles or labels.`,
        'podcast' // podcast özelliği için podcast ayarlarını kullan
      );
      
      const transcript = String(aiResponse.content || aiResponse);

      if(transcript && transcript !== '__RATE_LIMIT__' && transcript.length > 50){
        // Metinden kelime çıkar
        const words = transcript.match(/\b[A-Za-z]{5,}\b/g)||[];
        const uniqueWords = [...new Set(words.map(w=>w.toLowerCase()))].slice(0,5);
        PODCAST_EPISODES.push({
          id: i+1,
          emoji: t.emoji,
          title: t.title,
          level: t.level,
          duration: Math.ceil(transcript.split(' ').length / 130) + ' dk',
          desc: t.topic.charAt(0).toUpperCase() + t.topic.slice(1),
          transcript,
          words: uniqueWords
        });
      }
    ...
    if(i < shuffled.length-1) await new Promise(r=>setTimeout(r,2000));
  }
...
function initPodcastScreen() {
  const list = document.getElementById('podcastList');
  if(!PODCAST_EPISODES.length){
    generatePodcastEpisodes();
    return;
  }
  list.innerHTML = PODCAST_EPISODES.map(ep => `
    <div class="podcast-ep" id="ep${ep.id}" onclick="playEpisode(${ep.id})">
      <div class="pe-header">
        <div class="pe-thumb">${ep.emoji}</div>
        <div>
          <div class="pe-title">${ep.title}</div>
          <div class="pe-meta">📊 ${ep.level} • ⏱ ${ep.duration}</div>
        </div>
      </div>
      <div style="font-size:12px;color:var(--muted)">${ep.desc}</div>
    </div>`).join('') +
    '<button onclick="generatePodcastEpisodes()" style="width:100%;margin-top:10px;padding:12px;background:var(--bg3);border:1.5px dashed var(--border);border-radius:14px;font-size:13px;font-weight:700;cursor:pointer;color:var(--muted)">🔄 Yeni İçerikler Üret</button>';
}

function playEpisode(id) {
  const ep = PODCAST_EPISODES.find(e => e.id === id);
  if (!ep) return;
  
  if (currentEpisode) stopPodcast();
  currentEpisode = ep;
  
  document.querySelectorAll('.podcast-ep').forEach(el => el.classList.remove('playing'));
  document.getElementById('ep' + id).classList.add('playing');
  
  document.getElementById('podcastPlayerBar').style.display = '';
  document.getElementById('ppThumb').textContent = ep.emoji;
  document.getElementById('ppTitle').textContent = ep.title;
  document.getElementById('ppMeta').textContent = ep.level + ' • ' + ep.duration;
  
  // Transkript göster
  const transcriptEl = document.getElementById('podcastTranscript');
  transcriptEl.style.display = '';
  renderInteractiveTranscript(ep.transcript, ep.words);
  
  // TTS ile oku
  const utt = new SpeechSynthesisUtterance(ep.transcript);
  utt.lang = 'en-US';
  utt.rate = podcastSpeedRate;
  
  const totalWords = ep.transcript.split(' ').length;
  let wordsPassed = 0;
  utt.onboundary = e => {
    if (e.name === 'word') {
      wordsPassed++;
      const pct = Math.round((wordsPassed / totalWords) * 100);
      document.getElementById('ppFill').style.width = pct + '%';
      const totalSecs = Math.round(totalWords * 0.4 / podcastSpeedRate);
      const elapsed = Math.round(wordsPassed * 0.4 / podcastSpeedRate);
      document.getElementById('ppCurrentTime').textContent = formatTime(elapsed);
      document.getElementById('ppDuration').textContent = formatTime(totalSecs);
    }
  };
  utt.onend = () => {
    document.getElementById('ppPlayBtn').textContent = '▶ Yeniden Oynat';
    document.getElementById('ppFill').style.width = '100%';
  };
  
  speechSynthesis.speak(utt);
  document.getElementById('ppPlayBtn').textContent = '⏸ Duraklat';
}

function renderInteractiveTranscript(text, keywords) {
  const words = text.split(' ');
  const html = words.map(w => {
    const clean = w.toLowerCase().replace(/[^a-z]/g, '');
    const isKey = keywords.some(k => k.toLowerCase() === clean);
    const isLearned = learnedSet.has(clean);
    return `<span class="whl-word ${isKey ? 'unknown' : isLearned ? 'known' : ''}" onclick="podcastWordClick('${clean}')" title="${clean}">${w}</span>`;
  }).join(' ');
  document.getElementById('transcriptText').innerHTML = html;
}

async function podcastWordClick(word) {
  const response = await callGroqAPI(
    'You are a vocabulary teacher. Respond in Turkish, briefly.',
    `Explain the word "${word}" briefly: meaning, usage. One sentence example.`
  );
  showToast('📚 ' + word, response.slice(0, 80));
}

function togglePodcast() {
  const tb = document.getElementById('transcriptPlayBtn');
  if (speechSynthesis.paused) {
    ...
    document.getElementById('ppPlayBtn').textContent = '▶ Devam Et';
    if(tb) tb.textContent = '▶';
  } else if (currentEpisode) {
    playEpisode(currentEpisode.id);
  }
}

function stopPodcast() {
  speechSynthesis.cancel();
  document.getElementById('ppPlayBtn').textContent = '▶ Oynat';
  const tb2 = document.getElementById('transcriptPlayBtn'); if(tb2) tb2.textContent = '▶';
  document.getElementById('ppFill').style.width = '0%';
  currentEpisode = null;
  document.querySelectorAll('.podcast-ep').forEach(el => el.classList.remove('playing'));
}

function podcastRewind() {
  stopPodcast();
  if (currentEpisode) playEpisode(currentEpisode.id);
}

function podcastFwd() { /* TTS seek not supported, skip next */ nextEpisode(); }
function nextEpisode() {
  if (!currentEpisode) return;
  const nextId = (currentEpisode.id % PODCAST_EPISODES.length) + 1;
  playEpisode(nextId);
}

function setPodcastSpeed(speed, el) {
  podcastSpeedRate = speed;
...
function seekPodcast(e) {
  ...
  document.getElementById('ppFill').style.width = (pct * 100) + '%';
}
  ...
  if(btn){
    btn.addEventListener('click', async ()=>{
      ...
      if(result.outcome === 'accepted'){
        showToast('✅ Uygulama eklendi!', 'Ana ekranda artık mevcut');
...
// iOS için manuel talimat
if(/iPhone|iPad|iPod/.test(navigator.userAgent) && !navigator.standalone){
  window.addEventListener('load', ()=>{
    setTimeout(()=> showToast('📱 Ana ekrana ekle', 'Paylaş → Ana Ekrana Ekle seç'), 3000);
  });
}
...
function updateOfflineStatus() {
  ...
  if (navigator.onLine) {
    ...
    txt.textContent = 'İnternet bağlantısı var';
    bar.innerHTML = '<span>🟢</span> <span>İnternet bağlantısı var</span>';
  } else {
    bar.className = 'offline-status offline';
    bar.innerHTML = '<span>🔴</span> <span>Çevrimdışı - Önbellek kullanılıyor</span>';
  }
}

function updateCacheStats() {
  // Word cache
  const wordData = localStorage.getItem('wordMode_backup') || '';
  const wordSize = (wordData.length / 1024).toFixed(1);
  const progressData = JSON.stringify({wordStatus, learnedSet: [...learnedSet], spacedRepetition});
  const progressSize = (progressData.length / 1024).toFixed(1);
  const aiSize = (JSON.stringify(aiResponseCache).length / 1024).toFixed(1);
  
  const totalKB = parseFloat(wordSize) + parseFloat(progressSize) + parseFloat(aiSize);
  const maxKB = 5120; // 5MB
  const pct = Math.min(100, Math.round((totalKB / maxKB) * 100));
  
  document.getElementById('wordCacheSize') && (document.getElementById('wordCacheSize').textContent = wordSize + ' KB');
  document.getElementById('progressCacheSize') && (document.getElementById('progressCacheSize').textContent = progressSize + ' KB');
  document.getElementById('aiCacheSize') && (document.getElementById('aiCacheSize').textContent = aiSize + ' KB');
  document.getElementById('cachePercent') && (document.getElementById('cachePercent').textContent = pct + '%');
  document.getElementById('cacheFill') && (document.getElementById('cacheFill').style.width = pct + '%');
  document.getElementById('aiCacheStatus') && (document.getElementById('aiCacheStatus').textContent = Object.keys(aiResponseCache).length + ' yanıt');
  document.getElementById('aiCacheStatus') && document.getElementById('aiCacheStatus').classList.add('cached');
}
...
function cacheAllData() {
  ...
  try {
    ...
    localStorage.setItem('wordMode_offline_cache', JSON.stringify(backup));
    showToast('💾 Önbellek', 'Tüm veriler önbelleğe alındı!');
    updateCacheStats();
  } catch(e) {
    showToast('❌ Hata', 'Önbellek hatası: ' + e.message);
  }
}

function clearOfflineCache() {
  if (confirm('🗑️ Önbellek temizlensin mi?')) {
    localStorage.removeItem('wordMode_offline_cache');
    aiResponseCache = {};
    updateCacheStats();
    showToast('🗑️ Temizlendi', 'Önbellek temizlendi');
  }
}
...
async function installPWA() {
  if (pwaInstallPrompt) {
    ...
    const result = await pwaInstallPrompt.userChoice;
    if (result.outcome === 'accepted') {
      document.getElementById('pwaStatus').textContent = '✅ Uygulama kuruldu!';
      pwaInstallPrompt = null;
    }
  } else {
    document.getElementById('pwaStatus').textContent = '📱 Tarayıcı menüsünden "Ana ekrana ekle" seç';
  }
}
...
function saveOfflineSettings() {
  const settings = {
    ...
    cacheAI: document.getElementById('cacheAI')?.checked
  };
  localStorage.setItem('offlineSettings', JSON.stringify(settings));
}

// AI response caching wrapper
const originalCallGroq = callGroqAPI;
async function callGroqAPICached(system, prompt) {
  const cacheSettings = JSON.parse(localStorage.getItem('offlineSettings') || '{}');
  if (cacheSettings.cacheAI) {
    const key = btoa(system.slice(0,20) + prompt.slice(0,40));
    if (aiResponseCache[key]) return aiResponseCache[key];
    const result = await callGroqAPI(system, prompt);
    aiResponseCache[key] = result;
    return result;
  }
  return callGroqAPI(system, prompt);
}
...
const ROOMS = [
  {
    id: 'daily', emoji: '☕', title: 'Daily Chat Lounge', subtitle: 'Günlük konuşma pratiği',
    participants: ['Sarah', 'Alex', 'Mia'], level: 'All Levels', live: true,
    system: 'You are managing a friendly English conversation group. Multiple characters participate: Sarah (friendly), Alex (professional), Mia (teacher). Each response, pick ONE character to respond naturally. Keep responses SHORT. Format: "[Name]: message"'
  },
  {
    id: 'debate', emoji: '⚖️', title: 'English Debate Club', subtitle: 'Tartışma & argüman pratiği',
    participants: ['Jake', 'Emma'], level: 'Intermediate+', live: false,
    system: 'You are managing an English debate club. Two characters: Jake (casual, pro-change) and Emma (formal, traditional). They debate various topics. Pick one to respond each time. Format: "[Name]: message"'
  },
  {
    id: 'business', emoji: '💼', title: 'Business English Room', subtitle: 'İş İngilizcesi pratiği',
    participants: ['Mr. Chen', 'Ms. Park', 'Tom'], level: 'Intermediate', live: true,
    system: 'You are managing a business English room. Characters: Mr. Chen (CEO), Ms. Park (Marketing), Tom (intern). They discuss business topics professionally. Format: "[Name]: message"'
  },
  {
    id: 'travel', emoji: '✈️', title: 'Travel Stories', subtitle: 'Seyahat anıları & planlar',
    participants: ['Maria', 'David'], level: 'Beginner+', live: false,
    system: 'You are managing a travel English room. Characters: Maria (world traveler, Spanish) and David (adventure seeker, British). They share travel stories and tips. Format: "[Name]: message"'
  }
];

function initRoomsScreen() {
  const list = document.getElementById('roomList');
  list.innerHTML = ROOMS.map(r => `
    <div class="room-card ${r.live ? 'live' : ''}" onclick="joinRoom('${r.id}')">
      <div class="room-header">
        <div style="font-size:24px">${r.emoji}</div>
        <div style="flex:1">
          <div style="display:flex;align-items:center;gap:6px">
            <div class="room-title">${r.title}</div>
            <div class="room-tag ${r.live ? 'live' : ''}">${r.live ? '🔴 CANLI' : '💬 AI'}</div>
          </div>
        </div>
      </div>
      <div style="font-size:12px;color:var(--muted);margin-bottom:6px">${r.subtitle}</div>
      <div class="room-meta">
...
async function joinRoom(id) {
  ...
  document.getElementById('roomList').closest('.card').style.display = 'none';
  document.getElementById('roomInner').style.display = '';
  ...
  addRoomMsg('system', 'Oda', `🚪 "${room.title}" odasına katıldın! ${room.participants.join(', ')} burada.`);
  
  const opening = await callGroqAPI(
    room.system,
    'Start the conversation naturally. Welcome the new user who just joined.'
  );
  addRoomMsg('other', opening.split(':')[0] || room.participants[0], opening.includes(':') ? opening.split(':').slice(1).join(':').trim() : opening);
  roomHistory.push({ role: 'assistant', content: opening });
}

function addRoomMsg(type, author, text) {
  const div = document.createElement('div');
  div.className = 'room-msg ' + (type === 'self' ? 'self' : type === 'system' ? '' : 'other');
  
  if (type === 'system') {
    div.innerHTML = `<div style="text-align:center;font-size:12px;color:var(--purple);padding:6px 0">${text}</div>`;
  } else {
    div.innerHTML = `<div class="msg-author">${author}</div>${highlightEnglishWords(text)}`;
  }
  
  document.getElementById('roomMessages').appendChild(div);
  div.scrollIntoView({ behavior: 'smooth' });
}
...
async function sendRoomMsg() {
  ...
  addRoomMsg('self', 'Sen', msg);
  roomHistory.push({ role: 'user', content: msg });
  
  const messages = [
    { role: 'system', content: currentRoom.system + ' The user just joined. Keep responses natural and SHORT (1-2 sentences per character). Always stay in character.' },
    ...roomHistory.slice(-8)
  ];
  
  const response = await callGroqAPIWithHistory(messages);
  roomHistory.push({ role: 'assistant', content: response });
  
  // Parse character name from response
  const colonIdx = response.indexOf(':');
  const author = colonIdx > 0 && colonIdx < 20 ? response.slice(0, colonIdx).replace(/[\[\]]/g, '').trim() : currentRoom.participants[0];
  const text = colonIdx > 0 ? response.slice(colonIdx + 1).trim() : response;
  
  addRoomMsg('other', author, text);
  speak(text.replace(/<[^>]+>/g, ''), 'en-US');
}

function roomKeyDown(e) {
  if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); sendRoomMsg(); }
}

function leaveRoom() {
  currentRoom = null;
  roomHistory = [];
  document.getElementById('roomInner').style.display = 'none';
  document.getElementById('roomList').closest('.card').style.display = '';
}

// ══════════════════════════════════════════════════════════
// 🌅 GÜNLÜK PLAN DASHBOARD
// ══════════════════════════════════════════════════════════
let dailyXP = 0;
let dailyTasksDone = {};
const DAILY_QUOTES = [
  { text: "Bir dili öğrenmek, yeni bir pencere açmaktır.", author: "Frank Smith" },
  { text: "Her kelime, yeni bir dünyaya açılan bir kapıdır.", author: "Anonim" },
  { text: "Dil öğrenmek için en iyi zaman 20 yıl önceydi. İkinci en iyi zaman şimdi.", author: "Çin Atasözü" },
  { text: "Konuştuğun her dil, yaşadığın ayrı bir hayattır.", author: "Çek Atasözü" },
  { text: "Başarı, her gün küçük adımlar atmaktır.", author: "Anonim" },
  { text: "Zorluk olmadan ilerleme olmaz.", author: "Frederick Douglass" },
  { text: "Bugün yapman gerekeni yarına bırakma.", author: "Benjamin Franklin" }
];

function initDailyDashboard() {
  const now = new Date();
  const hour = now.getHours();
  const greetings = hour < 12 ? 'Günaydın! ☀️' : hour < 18 ? 'İyi öğleden sonralar! 🌤️' : 'İyi akşamlar! 🌙';
  document.getElementById('dailyGreeting').textContent = greetings;
  document.getElementById('dailyDate').textContent = now.toLocaleDateString('tr-TR', { weekday:'long', day:'numeric', month:'long' });
  ...
  // Streak
  const streakCount = parseInt(localStorage.getItem('wm_streak') || '0');
  document.getElementById('dailyStreakCount').textContent = streakCount;
  document.getElementById('dailyStreakMsg').textContent = streakCount >= 7 ? '🔥 Harika seri!' : streakCount >= 3 ? '💪 Devam et!' : 'Seriyi koru!';

  // XP sistemi
  const todayKey = 'xp_' + now.toISOString().slice(0,10);
  dailyXP = parseInt(localStorage.getItem(todayKey) || '0');
  updateXPDisplay();

  // Quote
  const q = DAILY_QUOTES[now.getDay() % DAILY_QUOTES.length];
  document.getElementById('dailyQuote').innerHTML = `<div class="dq-text">"${q.text}"</div><div class="dq-author">— ${q.author}</div>`;

  // Görevler
  const dateKey = now.toISOString().slice(0,10);
  dailyTasksDone = JSON.parse(localStorage.getItem('dailyTasks_' + dateKey) || '{}');
  renderDailyTasks();
...
function updateXPDisplay() {
  const level = Math.floor(dailyXP / 100) + 1;
  const xpInLevel = dailyXP % 100;
  document.getElementById('dailyXP').textContent = dailyXP;
  document.getElementById('dailyLevel').textContent = level;
  document.getElementById('dailyXPProgress').textContent = xpInLevel + ' / 100 XP';
  document.getElementById('dailyXPBar').style.width = xpInLevel + '%';
}

function addXP(amount, reason) {
  const todayKey = 'xp_' + new Date().toISOString().slice(0,10);
  dailyXP += amount;
  localStorage.setItem(todayKey, dailyXP);
  updateXPDisplay();
...
function renderDailyTasks() {
  ...
  const tasks = [
    ...
    { id: 'story', icon: '📖', title: 'AI hikaye oku', desc: 'Öğrendiğin kelimelerle hikaye', xp: 15, done: dailyTasksDone.story, action: () => generateStory() },
...
// ══════════════════════════════════════════════════════════
// saveProgress'e analitik kaydı ekle (monkey patch)
const _origSaveProgress = saveProgress;
function saveProgress() {
  _origSaveProgress();
  // Bugün öğrenilen kelime sayısını kaydet
  const today = new Date().toISOString().slice(0,10);
...
Format as JSON: {"main": "...", "scene1": "...", "scene2": "...", "hook": "...", "emoji": "single emoji"}`;
    ...
    const response = await callAI('You are a visual English vocabulary teacher. Respond in Turkish.', prompt, 'visual');
    ...
    let parsed = { main: desc, scene1: '', scene2: '', hook: '', emoji: '🖼️' };
    try {
    ...
    const usedModel = response.model || getAIModel('visual');
    const usedTokenLimit = response.tokenLimit || getAITokenLimit('visual');
    const modelNames = {
      'groq': 'Groq Llama 3.3',
      'openai': 'OpenAI GPT-4o-mini',
      'claude': 'Claude 3.5 Sonnet',
    ...
    resultEl.innerHTML = `
      <div class="word-visual-card">
        ...
        <div class="wv-body">
          ...
          <div class="wv-desc">${parsed.main || desc}</div>
          <div class="wv-scenes">
  ...
  reader.onload = e => {
    try {
      ...
      for (let r = range.s.r + 1; r <= range.e.r; r++) {
        ...
        parsed.push({ word, tr, sentence, sentenceTr });
      }
...
function saveMultiLists() {
  ...
  try {
    localStorage.setItem('multiLists', JSON.stringify(toSave));
...
async function doCloudBackup() {
  ...
  if (selectedCloudProvider === 'gdrive') {
    ...
    const a = document.createElement('a');
    a.href = url;
    a.download = `wordmode_backup_${new Date().toISOString().slice(0,10)}.json`;
    a.click();
    ...
    const a = document.createElement('a');
    a.href = url;
    a.download = `wordmode_backup.json`;
    a.click();
...
function applyBackupData(data) {
  ...
  if (!confirm(`Geri yüklensin mi?\n${data.learnedSet.length} öğrenilmiş kelime\nTarih: ${data.exportDate?.slice(0,10) || '?'}`)) return;

  // İlerleme verisini uygula
  data.learnedSet.forEach(w => learnedSet.add(w));
  Object.assign(wordStatus, data.wordStatus || {});
  Object.assign(spacedRepetition, data.spacedRepetition || {});
  ...
  saveProgress();
  showToast('✅ Başarılı', `${data.learnedSet.length} kelime geri yüklendi!`);
...
function renderGameScores() {
  ...
  const games = [
    ...
    { key: 'listening', icon: '🔊', name: 'Dinleme Testi', unit: 'puan' }
  ];
  ...
  listEl.innerHTML = weak.slice(0,20).map(w=>{
    ...
    return `<div style="background:var(--bg2);border-radius:12px;padding:12px;border-left:3px solid ${barColor}">
      ...
      <div style="display:flex;gap:6px;margin-top:8px">
        <button onclick="explainWord('${w.word}','weaknessAIResult')" style="flex:1;padding:6px;background:var(--bg3);border:none;border-radius:8px;font-size:11px;font-weight:700;cursor:pointer;color:var(--text)">🤖 Açıkla</button>
...
async function processPDFFile(file){
  ...
  try {
    ...
    // 200KB limit — büyük dosyaları kırp
    const savedText = text.length > 200000 ? text.substring(0, 200000) : text;
    try {
      localStorage.setItem('book_text_'+bookId, savedText);
      localStorage.setItem('book_meta_'+bookId, JSON.stringify(meta));
    } catch(e){
      showToast('⚠️ Depolama dolu', 'Eski kitapları silin');
      showPDFProgress(false);
      return;
    }

    showPDFProgress(true, '✅ Hazır!', 100);
    setTimeout(()=>{ showPDFProgress(false); renderPDFBookList(); }, 800);
...
function renderPDFBookList(){
  ...
  const books = [];
  Object.keys(localStorage).forEach(k=>{
    if(k.startsWith('book_meta_pdf_')){
      try{
        const meta = JSON.parse(localStorage.getItem(k));
        if(meta.isPDF) books.push({id: k.replace('book_meta_',''), ...meta});
      }catch(e){}
    }
  });
...
function openPDFBook(bookId){
  ...
  if(!metaRaw || !text){ showToast('❌ Kitap bulunamadı',''); return; }
...
async function fireNotification(title, body){
  ...
  try {
    ...
    // reg.showNotification direkt çalışır — controller gerekmez
...
(function(){
  ...
  function getWord(el){
    ...
    // genel metin node — herhangi bir kart/cümle içindeki text
    if(el.closest&&el.closest('.word-card,.card,.conv-msg,.chat-msg,.correction-box,#storyContent,#accentAnalysisResult,#coachFeedback,#pronunResult,#visualResult,#songResult,.wv-body,.song-lyrics')){
      const txt=(el.textContent||'').trim();
      if(txt&&/^[A-Za-z]{2,}$/.test(txt)) return txt;
    }
    return null;
  }
...
function syncLearnedWordsWithMainArray() {
  ...
  learnedWords.forEach(learned => {
    ...
    const wordIndex = words.findIndex(w => w.word && w.word.toLowerCase() === learned.word.toLowerCase());
    if(wordIndex !== -1) {
    ...
    <div style="background:var(--card);border-radius:20px;width:90%;max-width:450px;max-height:80vh;overflow:hidden;border:1px solid var(--border);box-shadow:0 20px 60px rgba(0,0,0,.6)">
      ...
      <div style="max-height:60vh;overflow-y:auto;padding:16px">
        ${allWords.length === 0 ? 
          ...
          allWords.map(w => `
            <div style="display:flex;justify-content:space-between;padding:12px;background:var(--bg2);
              ...
              <div style="display:flex;align-items:center">
                <span style="font-size:11px;padding:4px 10px;background:${w.level === 'mastered' ? '#166534' : '#1e3a5f'};
                  color:${w.level === 'mastered' ? '#4ade80' : '#93c5fd'};border-radius:6px;font-weight:700">
                  ${w.level === 'mastered' ? '⭐' : '📖'}
                </span>
              </div>
            </div>
...
function saveAPIKeysSettings(){
  // API anahtarlarını inputlardan al
  apiKeys.groq = document.getElementById('apiKeyGroq')?.value.trim() || '';
  ...
  if(!apiKeys.groq && !apiKeys.openai && !apiKeys.claude && !apiKeys.gemini && !apiKeys.openrouter){
...
function loadAPIKeys() {
  ...
  if(stored) {
    try {
      ...
      console.log('🔑 API anahtarları yüklendi');
    } catch(e) {
...
async function callOpenRouterAPI(systemPrompt, userMessage, maxTokens = 500) {
  ...
  // ÜCRETSİZ MODELLER LİSTESİ (öncelik sırasına göre)
  const freeModels = [
    'google/gemini-2.0-flash-exp:free',           // En güçlü ücretsiz
    'meta-llama/llama-3.3-70b-instruct:free',     // Llama 3.3 70B ücretsiz
    'google/gemini-flash-1.5:free',               // Gemini Flash 1.5
    'meta-llama/llama-3.1-8b-instruct:free',      // Llama 3.1 8B
    'google/gemini-flash-1.5-8b:free',            // Gemini Flash 8B
    'nousresearch/hermes-3-llama-3.1-405b:free',  // Hermes 405B (bazı zamanlarda)
    'mistralai/mistral-7b-instruct:free'          // Mistral 7B
  ];
  
  // Rastgele bir ücretsiz model seç (yük dengelemesi için)
  const selectedModel = freeModels[Math.floor(Math.random() * Math.min(3, freeModels.length))];
  
  console.log('🌐 OpenRouter API çağrısı başlatılıyor...');
  console.log('Model:', selectedModel, '(ÜCRETSİZ)');
  console.log('Max tokens:', maxTokens);
  ...
  try {
    const response = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        temperature: 0.7,
        max_tokens: maxTokens,
        // Ücretli modellere düşmeyi engelle
        route: 'fallback',
        fallback_models: freeModels // Sadece ücretsiz modellere fallback
      })
    });
...
function importAllData(event){
  ...
  reader.onload = function(e){
    try {
      ...
      // Onay iste
      if(!confirm('⚠️ Mevcut verileriniz silinecek ve yedek dosya yüklenecek. Devam etmek istiyor musunuz?')){
        return;
      }
      
      // Verileri yükle
      if(importData.learnedWords){
        learnedWords = importData.learnedWords;
        saveLearnedWords();
      }
      
      if(importData.aiTokenSettings){
        aiTokenSettings = importData.aiTokenSettings;
        localStorage.setItem('aiTokenSettings', JSON.stringify(aiTokenSettings));
      }
      
      if(importData.spacedRepetition){
        spacedRepetition = importData.spacedRepetition;
        localStorage.setItem('spacedRepetition', JSON.stringify(spacedRepetition));
      }
      
      if(importData.learnedSet){
        learnedSet = new Set(importData.learnedSet);
        localStorage.setItem('learnedSet', JSON.stringify([...learnedSet]));
      }
      
      if(importData.wordStatus){
        wordStatus = importData.wordStatus;
        localStorage.setItem('wordStatus', JSON.stringify(wordStatus));
      }
      ...
      if(importData.streak !== undefined){
        streak = importData.streak;
        localStorage.setItem('streak', streak.toString());
      }
      
      if(importData.correctCount !== undefined){
        correctCount = importData.correctCount;
        localStorage.setItem('correctCount', correctCount.toString());
      }
      
      showToast('✅ İçe Aktarıldı!', 'Yedek başarıyla yüklendi');
      console.log('📥 Veri içe aktarıldı');
      ...
      showToast('⚠️ Hata', 'Dosya okunamadı veya geçersiz');
    }
  };
