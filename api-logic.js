  ...
  modal.innerHTML = `
    ...
    ">
      ...
      <div id="modalExplanationContent" style="
        ...
        margin-bottom: 16px;
      ">${makeModalWordsClickable(explanation)}</div>
...
async function renderLearn(){
  ...
  // 9. Otomatik ses çalma - SADECE KELIME DEĞİŞTİĞİNDE
  // lastRenderedWord ile kontrol et
  if(autoPlayAudio && window.lastRenderedWord !== item.word){
    ...
    // autoPlayAudio kapalıysa lastRenderedWord'ü güncelle
    window.lastRenderedWord = item.word;
  }
}
  ...
  for(let i = visibleStart; i < visibleEnd; i++){
    ...
    itemDiv.innerHTML = `
      ...
      <div class="wi-body">
        ...
        <div class="wi-tr">${(w.tr && w.tr !== 'undefined' && w.tr.trim() !== '') ? w.tr : (w.sentenceTr || 'çeviri yok')}${w.phonetic ? ' · <span style="color:var(--purple)">' + w.phonetic + '</span>' : ''}</div>
    	${sentPreview}
      </div>
