...
Format as JSON: {"main": "...", "scene1": "...", "scene2": "...", "hook": "...", "emoji": "single emoji"}`;
    ...
    const response = await callAI(getPrompt('visual'), prompt, 'visual');
...
4. **Collocations** (kelime öbekleri): 3-5 common FULL PHRASES (not just the word alone) with this word
...
**Collocations:**
- full phrase here (türkçe)
- another complete phrase (türkçe)

IMPORTANT: For Collocations, write the COMPLETE phrase, not just "${word}". Example: "take a break" not "take"`;

    const response = await callAI(getPrompt('relations'), prompt, 'explain');
    
    // Sonuçları göster
    const resultDiv = content.querySelector('div[style*="padding:20px"]');
    const fontSize = getAIFontSize();
    resultDiv.innerHTML = `
      <div style="font-size:${fontSize}px;line-height:2;color:var(--text);white-space:pre-wrap">${response.content}</div>
