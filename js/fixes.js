...
(function(){
  ...
  function stripDup(root){
    try{
      ...
      // 1) v13/v15 meta tekrarları (v14 kalır)
      ...
      // 6c) "📚 Cümle Ailesi" paneli (#wmV21FamilyCard) → düğmeye gizle (toggle)
      collapseToggle('#wmV21FamilyCard', 'wm-fam-toggle', '📚 Cümle Ailesi', '📚 Cümle Ailesini Gizle');

      // 7) SARI bar: "#wmFixedLearnedBar" içindeki açıklama YAZISINI (.wm-fixed-status) kaldır
      //    (buton #wmFixedLearnedBtn KALIR; aşağıda yeniden konumlanır)
      ...
      // 8) YEŞİL SR durum kutusu (.wm-sentence-status): ekranı doldurmasın → gizle,
      //    küçük "📊 Durum" butonuyla aç/kapa yapılır. (collapseGreenBox)
      collapseGreenBox(scope);

      // 9) Aksiyon satırı: [liste adı] · [🚀 Öğrenmeye Başladım] · [🏆 Gerçek Hayatta] yan yana
      arrangeActionRow();
    }catch(e){}
  }

  /* Genel: bir paneli gizle + hemen önüne aç/kapa butonu koy */
  function collapseToggle(selector, btnClass, labelClosed, labelOpen){
    try{
      var el = document.querySelector(selector);
      if(!el) return;
      if(el.getAttribute('data-wm-collapsed')==='1') return;
      el.setAttribute('data-wm-collapsed','1');
      el.style.display = 'none';
      var btn = document.createElement('button');
      btn.type = 'button';
      btn.className = btnClass;
      btn.textContent = labelClosed;
      btn.style.cssText = 'display:inline-flex;align-items:center;gap:5px;margin:8px 0;padding:6px 14px;border-radius:999px;border:1.5px solid var(--border,#334155);background:var(--bg3,#1b2230);color:var(--text,#cbd5e1);font-size:12px;font-weight:800;cursor:pointer;';
  ...
  /* YEŞİL kutuyu küçült: gizle + toggle buton ekle */
  function collapseGreenBox(scope){
    try{
      var box = (scope||document).querySelector('#wordCard .wm-sentence-status')
             || document.querySelector('#wordCard .wm-sentence-status');
      if(!box) return;
      if(box.getAttribute('data-wm-collapsed')==='1') return;  // zaten işlendi
      box.setAttribute('data-wm-collapsed','1');
      box.style.display = 'none';                              // başlangıçta gizli

      // toggle buton (kutudan hemen önce)
      var btn = document.createElement('button');
      btn.type = 'button';
      btn.className = 'wm-status-toggle';
      btn.textContent = '📊 Durum';
      btn.style.cssText = 'display:inline-flex;align-items:center;gap:5px;margin:6px 0;padding:5px 12px;border-radius:999px;border:1.5px solid var(--border,#334155);background:var(--bg3,#1b2230);color:var(--text,#cbd5e1);font-size:12px;font-weight:800;cursor:pointer;';
      btn.addEventListener('click', function(){
        var open = box.style.display !== 'none';
        box.style.display = open ? 'none' : '';
        btn.textContent = open ? '📊 Durum' : '📊 Durumu Gizle';
      });
      box.parentNode.insertBefore(btn, box);
    }catch(e){}
  }

  /* Aksiyon butonlarını tek satıra diz: liste adı + Öğrenmeye + Gerçek Hayatta + Durum */
  function arrangeActionRow(){
    try{
      var card = document.getElementById('wordCard');
      if(!card) return;
      var startBtn = document.getElementById('wmFixedLearnedBtn');   // 🚀 Öğrenmeye Başladım
      var realBtn  = document.getElementById('wmSentenceRealUseBtn'); // 🏆 Gerçek Hayatta
      var label    = card.querySelector('.wc-label');                // aktif liste adı (cumleler)
      if(!startBtn && !realBtn) return;                              // butonlar henüz yok

      var row = document.getElementById('wmActionRow');
      if(!row){
        row = document.createElement('div');
        row.id = 'wmActionRow';
        // TEK SATIR: nowrap + taşarsa yatay kaydır
        row.style.cssText = 'display:flex;align-items:center;gap:8px;flex-wrap:nowrap;overflow-x:auto;'
          + '-webkit-overflow-scrolling:touch;margin:10px 0;padding-bottom:2px;';
        card.insertBefore(row, card.firstChild);
      } else {
        // var olan satırı da tek-satıra zorla (eski wrap stilini ez)
        row.style.flexWrap = 'nowrap';
        row.style.overflowX = 'auto';
      }

      // bir öğeyi satıra alıp kompakt yap (kendi width:100%/flex/min-width stilini ez)
      function place(el, isLabel){
        if(!el || el.parentElement === row) return;
        if(isLabel){
          el.style.cssText = 'font-size:13px;font-weight:900;color:var(--text,#e2e8f0);margin:0;white-space:nowrap;flex:0 0 auto;';
        } else {
          // stylesheet'teki width:100% / min-width:154px!important ezilmeli → important ile
          ...
          el.style.setProperty('padding', '9px 12px', 'important');
        }
        row.appendChild(el);
      }

      place(label, true);                       // cumleler (en başta)
      if(startBtn){
        place(startBtn, false);                 // 🚀 Öğrenmeye Başladım
        var bar = document.getElementById('wmFixedLearnedBar');
        if(bar && !bar.querySelector('#wmFixedLearnedBtn')) bar.remove();
      }
      place(realBtn, false);                     // 🏆 Gerçek Hayatta
      place(document.querySelector('.wm-status-toggle'), false);  // 📊 Durum
    }catch(e){}
  }
