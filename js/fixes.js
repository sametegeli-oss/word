...
(function () {
  ...
  function refreshWordScreen() {
    ...
    // Her şey DOM'a yazıldı → GÖRÜNÜR YAPMADAN ÖNCE senkron temizle (mükerrer
    // metalar/butonlar/başlıklar kaldırılsın) ki dağınık ham hâl hiç parlamasın.
    // (MutationObserver'ın 40ms debounce'unu beklemeyiz; o yine arkadan koşar.)
    try { if (window.WM_stripDup) window.WM_stripDup(document.getElementById('sc-word') || card); } catch (e) {}

    // Temizlik bittikten sonra tek frame'de fade ile göster (parça parça yerine bütün)
    if (card) {
      requestAnimationFrame(function(){
        // ikinci bir senkron geçiş: renderLearn/sentence-mode geç eklediyse de temiz açılsın
        try { if (window.WM_stripDup) window.WM_stripDup(document.getElementById('sc-word') || card); } catch (e) {}
