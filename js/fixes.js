    ...
    var _wmLSI = async function(sentence, word){
      ...
      try{
        ...
        if(embed && /^https?:\/\//i.test(String(embed))){
          img.onload=function(){ wrap.style.display='block'; if(credit) credit.innerHTML='📷 Kaynak dosya'; };
          img.onerror=function(){ wrap.style.display='none'; };
          img.src=embed; return;
        }
      }catch(e){}

      var key=ckey(word, sentence);
      wrap.style.display='none'; img.removeAttribute('src'); if(credit) credit.innerHTML='';

      // 2) cache
      var cached=await cacheGet(key);
      if(cached!==undefined){
        if(cached==='__NONE__'){ wrap.style.display='none'; return; }
        img.onload=function(){ wrap.style.display='block'; if(credit) credit.innerHTML='📷 Önbellek'; };
        img.onerror=function(){ wrap.style.display='none'; };
        img.src=cached; return;
      }
      ...
      if(url){
        img.onload=function(){ wrap.style.display='block'; if(credit) credit.innerHTML='📷 Wikipedia / Openverse / Commons'; };
        img.onerror=function(){ wrap.style.display='none'; };
        img.src=url; cacheSet(key, url);
      }else{
