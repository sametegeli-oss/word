...
(function () {
  ...
  async function loadDict() {
    // Legacy'nin lazy yükleyicisini de tetikle (varsa) — popup anlam yarışını önler
    try { if (typeof window.WM_loadSozlukExternal === 'function') window.WM_loadSozlukExternal(); } catch (e) {}

    // Map zaten zenginse (çok kelime + bilinen bir kelime anlamıyla varsa) tekrar yükleme
    var m = window.WM_SOZLUK_MEANING_MAP;
    var rich = m && Object.keys(m).length > 500;
    var sample = rich && (m['abandon'] || m['organization']);
    if (rich && sample && (sample.meanings || sample.anlam1)) return;
