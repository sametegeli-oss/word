...
(function () {
  ...
  function refreshWordScreen() {
    ...
    // SADECE wordCard'ı değil TÜM #sc-word ekranını gizle: Benzer Cümleler,
    // Cümle Ailesi, kırmızı #sentenceModeMeta gibi paneller wordCard DIŞINDA ama
    // sc-word İÇİNDE üretiliyor; bunlar gizlenmeyince ham hâlleri parlıyordu.
    var hideEl = screen || card;
    if (hideEl) { hideEl.style.opacity = '0'; hideEl.style.animation = 'none'; }
    if (card && card !== hideEl) { card.style.animation = 'none'; }
    ...
    var frames = 0, MAX = 6;   // ~6 frame ≈ 100ms; geç panelleri yakalamaya yeter
    (function settle(){
      doStrip();
      frames++;
      if (frames < MAX) { requestAnimationFrame(settle); return; }
      // DOM oturdu → tek seferde fade ile göster (parça parça yerine bütün)
      if (hideEl) {
        if (card) card.style.animation = 'wmFade .28s ease both';
...
(function(){
  ...
  function injectLayoutGuard(){
    ...
    var css = ''
      ...
      // Cümle Ailesi / Cümle Koçu mini gibi paneller renderLearn'den SONRA
      // asenkron basılıyor (callAI sonrası ~140ms). Önce #wordCard içine GÖRÜNÜR
      // basılıp sonra #wmWordSheet'e taşınıyordu → "açılıp toplanma".
      // ÇÖZÜM: bu panelleri HER YERDE gizle; SADECE #wmWordSheet içindeyken göster.
      // Böylece wordCard'a basıldığı an görünmez, panele taşınınca görünür olur.
      + '#wmV21FamilyCard,#wmV21SentenceMini{display:none !important;}'
      + '#wmWordSheet #wmV21FamilyCard{display:block !important;}';
