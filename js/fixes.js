...
(function () {
  ...
  function refreshWordScreen() {
    ...
    try {
      ...
      // POS renklendirmeyi de kart görünmeden ÖNCE dene (cache varsa anında, renk sıçraması olmaz)
      try { if (window.WM_colorizePOSSilent) window.WM_colorizePOSSilent(); } catch (e) {}
...
/* ════════════════════════════════════════════════════════════════════
   SADE KELİME EKRANI (Busuu tarzı)  —  bağımsız modül
   - Mevcut sc-word kartı AYNEN kalır; sade mod opsiyonel bir KATMANDIR.
   - DOM yeniden yazılmaz: body.wm-simple sınıfı + CSS ile gereksiz parçalar
     gizlenir, kalanlar yeniden düzenlenir. renderLearn/POS/görsel zinciri korunur.
   - Kalan: görsel + CEFR seviye chip + RENKLİ cümle + Türkçe + küçük düğmeler.
   - Gizlenen: büyük kelime, kelime Türkçesi, fonetik, SR rozeti, liste etiketi,
     gramer (🏗️) chip'i.
   - ⋯ düğmesi → alttan açılır boş sheet İSKELETİ (butonlar sonra eklenecek).
   - Ayar WMStore'da kalıcı: anahtar 'wm_simple_mode'.
   ════════════════════════════════════════════════════════════════════ */
(function(){
  'use strict';
  var KEY = 'wm_simple_mode';

  /* ---- ayar oku/yaz (WMStore, yoksa localStorage fallback) ---- */
  function readPref(){
    try{
      if(window.WMStore && WMStore.get){
        return Promise.resolve(WMStore.get(KEY)).then(function(v){
          if(v==null){ try{ return localStorage.getItem(KEY)==='1'; }catch(e){ return false; } }
          return v==='1' || v===1 || v===true;
        }).catch(function(){ return false; });
      }
    }catch(e){}
    try{ return Promise.resolve(localStorage.getItem(KEY)==='1'); }catch(e){}
  ...
  /* ---- sade mod aç/kapa ---- */
  function apply(on){
    try{ document.body.classList.toggle('wm-simple', !!on); }catch(e){}
    // Sade modda cümle görünür olduğu için POS renklendirmeyi tetikle (cache'liyse anında)
    if(on){ try{ if(window.WM_colorizePOSSilent) setTimeout(window.WM_colorizePOSSilent, 120); }catch(e){} }
  }
  window.WM_setSimpleMode = function(on){ writePref(on); apply(on); };
  window.WM_toggleSimpleMode = function(){
    var on = !document.body.classList.contains('wm-simple');
    window.WM_setSimpleMode(on);
    return on;
  };
  window.WM_isSimpleMode = function(){ return document.body.classList.contains('wm-simple'); };

  /* ---- CSS: sade düzen + bottom sheet ---- */
  function injectCSS(){
    if(document.getElementById('wm-simple-css')) return;
    var css = ''
      // ── Sade modda GİZLENENLER (sc-word kartı içinde) ──
      + 'body.wm-simple #sc-word .wc-word,'
      + 'body.wm-simple #sc-word .wc-tr,'
      + 'body.wm-simple #sc-word .wc-phonetic,'
      + 'body.wm-simple #sc-word .wm-v15-grammar,'        /* 🏗️ gramer chip */
      + 'body.wm-simple #sc-word .wc-label{display:none !important;}'
      // SR seviye rozeti (renderLearn inline div: "🌱 Yeni / 📅 ...") — wc-label'dan hemen sonraki
      // kelime-SR bloğunu gizlemek için işaret sınıfı JS ile eklenir (.wm-sr-row)
      + 'body.wm-simple #sc-word .wm-sr-row{display:none !important;}'
      // ── Sade modda KALAN meta: yalnız CEFR seviye chip'i, ortalı ──
      + 'body.wm-simple #sc-word .wm-v15-meta{justify-content:center !important;display:flex !important;margin:0 0 10px !important;}'
      // ── Cümle: ana eleman, büyük + ortalı ──
      + 'body.wm-simple #sc-word .wc-sent{font-size:21px !important;line-height:1.45 !important;text-align:center !important;font-weight:700 !important;margin:2px 0 8px !important;}'
      + 'body.wm-simple #sc-word .wc-sent-tr{text-align:center !important;color:var(--muted,#94a3b8) !important;font-size:14px !important;margin:0 0 4px !important;}'
      // ── ⋯ düğmesi sade modda görünür, normalde gizli ──
      + '.wm-more-btn{display:none;}'
      + 'body.wm-simple .wm-more-btn{display:inline-flex !important;align-items:center;justify-content:center;}'
      // ── Bottom sheet ──
      + '.wm-sheet-backdrop{position:fixed;inset:0;background:rgba(0,0,0,.5);z-index:9998;opacity:0;pointer-events:none;transition:opacity .25s ease;}'
      + '.wm-sheet-backdrop.open{opacity:1;pointer-events:auto;}'
      + '.wm-sheet{position:fixed;left:0;right:0;bottom:0;z-index:9999;background:var(--bg2,#0f1623);border-top:1px solid var(--border,#1e293b);'
      + 'border-radius:18px 18px 0 0;padding:10px 16px calc(16px + env(safe-area-inset-bottom)) 16px;transform:translateY(105%);transition:transform .28s cubic-bezier(.22,1,.36,1);max-height:70vh;overflow-y:auto;}'
      + '.wm-sheet.open{transform:translateY(0);}'
      + '.wm-sheet-grip{width:36px;height:4px;border-radius:99px;background:var(--border,#334155);margin:4px auto 14px;}'
      + '.wm-sheet-title{font-size:13px;font-weight:800;color:var(--muted,#94a3b8);margin:0 0 12px;text-align:center;}'
      + '.wm-sheet-grid{display:grid;grid-template-columns:1fr 1fr;gap:10px;}'
      + '.wm-sheet-empty{grid-column:1/-1;text-align:center;color:var(--muted,#64748b);font-size:13px;padding:18px 0;}'
      + '.wm-sheet-btn{height:46px;border-radius:12px;background:var(--bg3,#1b2230);border:1.5px solid var(--border,#334155);color:var(--text,#e2e8f0);'
      + 'font-size:13px;font-weight:700;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:6px;}'
      + '.wm-sheet-btn:active{transform:scale(.97);}'
      // Açık tema uyumu
      + 'body.light-mode .wm-sheet-backdrop{background:rgba(0,0,0,.35);}';
    var s = document.createElement('style');
    s.id = 'wm-simple-css';
    s.textContent = css;
    document.head.appendChild(s);
  ...
  /* ---- SR seviye satırını işaretle (renderLearn her çizimde yeniden basar) ----
     renderLearn kartın başına: wc-label → SR rozet div'i → meta → kelime sırasıyla basar.
     SR rozet div'i "Henüz çalışılmadı" / "🌱 Yeni" gibi metin içerir; onu sınıfla işaretle. */
  function markSrRow(card){
    try{
      var label = card.querySelector('.wc-label');
      if(!label) return;
      var n = label.nextElementSibling;
      // wc-label'dan sonraki ilk DIV genelde SR rozet satırı (içinde rozet span'i var)
      if(n && n.tagName==='DIV' && !n.classList.contains('wm-v15-meta') &&
         !n.classList.contains('wc-word') && /\d{1,2}\.\d|Yeni|çalışılmadı|Tekrar|Başlangıç|Orta|İyi|Güçlü|Uzman|📅|⏰|🌱|❌/.test(n.textContent||'')){
        n.classList.add('wm-sr-row');
      }
    }catch(e){}
  }

  /* ---- ⋯ düğmesi + sheet enjeksiyonu ---- */
  function ensureSheet(){
    if(document.getElementById('wm-sheet')) return;
    var bd = document.createElement('div');
    bd.className = 'wm-sheet-backdrop';
    bd.id = 'wm-sheet-backdrop';
    bd.addEventListener('click', closeSheet);

    var sheet = document.createElement('div');
    sheet.className = 'wm-sheet';
    sheet.id = 'wm-sheet';
    sheet.innerHTML =
      '<div class="wm-sheet-grip"></div>'
      + '<div class="wm-sheet-title">Daha fazla</div>'
      + '<div class="wm-sheet-grid" id="wm-sheet-grid">'
      + '  <div class="wm-sheet-empty">Yakında: telaffuz, gramer, açıklama…</div>'
      + '</div>';
    document.body.appendChild(bd);
    document.body.appendChild(sheet);
  }
  function openSheet(){ ensureSheet(); var b=document.getElementById('wm-sheet-backdrop'),s=document.getElementById('wm-sheet'); if(b)b.classList.add('open'); if(s)s.classList.add('open'); }
  function closeSheet(){ var b=document.getElementById('wm-sheet-backdrop'),s=document.getElementById('wm-sheet'); if(b)b.classList.remove('open'); if(s)s.classList.remove('open'); }
  window.WM_openWordSheet = openSheet;
  window.WM_closeWordSheet = closeSheet;
  // Dışarıdan buton eklemek için yardımcı (ileride kullanılacak)
  window.WM_addSheetButton = function(label, icon, onClick){
    ensureSheet();
    var grid = document.getElementById('wm-sheet-grid'); if(!grid) return;
    var empty = grid.querySelector('.wm-sheet-empty'); if(empty) empty.remove();
    var b = document.createElement('button');
    b.className = 'wm-sheet-btn';
    b.innerHTML = (icon? icon+' ':'') + label;
    b.addEventListener('click', function(){ try{ onClick&&onClick(); }catch(e){} closeSheet(); });
    grid.appendChild(b);
  };

  /* ---- nav bar'ına ⋯ düğmesi ekle (sade modda görünür) ---- */
  function ensureMoreBtn(card){
    try{
      // nav grid: prevWord ◀ ... navNextWord ▶ içeren satır
      var navBtn = card.querySelector('button[onclick*="navNextWord"]');
      if(!navBtn) return;
      var bar = navBtn.parentElement;            // ileri butonun kapsayıcısı
      var grid = bar; // grid satırı (auto 1fr auto)
      if(!grid || grid.querySelector('.wm-more-btn')) return;
      var more = document.createElement('button');
      more.className = 'wm-more-btn';
      more.type = 'button';
      more.setAttribute('aria-label','Daha fazla');
      more.textContent = '⋯';
      more.style.cssText = 'padding:10px 12px;background:var(--bg3,#1b2230);border:1.5px solid var(--border,#475569);border-radius:10px;color:var(--accent,#93c5fd);font-size:18px;font-weight:800;cursor:pointer;line-height:1;';
      more.addEventListener('click', function(e){ e.preventDefault(); openSheet(); });
      // ileri butonun hemen SOLUNA koy
      grid.insertBefore(more, navBtn);
    }catch(e){}
  }

  /* ---- kart her çizildiğinde sade-mod işaretlerini uygula ---- */
  function decorate(){
    var card = document.getElementById('wordCard');
    if(!card) return;
    markSrRow(card);
    if(window.WM_isSimpleMode()) ensureMoreBtn(card);
  }

  /* ---- kart değişimini izle (renderLearn innerHTML'i yeniden yazar) ---- */
  function watch(){
    var card = document.getElementById('wordCard');
    if(!card){ setTimeout(watch, 500); return; }
    decorate();
    try{
      var t=null;
      new MutationObserver(function(){
    ...
    }catch(e){ setInterval(decorate, 1000); }
  }

  /* ---- ayar ekranına toggle satırı ekle ---- */
  function injectToggleRow(){
    var settings = document.getElementById('sc-settings');
    if(!settings || document.getElementById('wm-simple-toggle-row')) return;
    var row = document.createElement('div');
    row.id = 'wm-simple-toggle-row';
    row.style.cssText = 'display:flex;align-items:center;justify-content:space-between;gap:12px;padding:14px 16px;margin:10px 0;background:var(--bg3,#1b2230);border:1.5px solid var(--border,#334155);border-radius:14px;';
    row.innerHTML =
      '<div style="flex:1">'
      + '<div style="font-weight:800;font-size:14px;color:var(--text,#e2e8f0)">📖 Sade Kelime Ekranı</div>'
      + '<div style="font-size:12px;color:var(--muted,#94a3b8);margin-top:2px">Yalnız cümle + Türkçe + görsel (Busuu tarzı)</div>'
      + '</div>'
      + '<button id="wm-simple-toggle-btn" type="button" role="switch" aria-checked="false" '
      + 'style="width:52px;height:30px;border-radius:99px;border:none;cursor:pointer;background:#334155;position:relative;transition:background .2s;flex:0 0 auto;">'
      + '<span id="wm-simple-toggle-knob" style="position:absolute;top:3px;left:3px;width:24px;height:24px;border-radius:50%;background:#fff;transition:left .2s;"></span>'
      + '</button>';
    // ayar ekranının başına ekle (en görünür)
    settings.insertBefore(row, settings.firstChild ? settings.firstChild.nextSibling : null);

    var btn = row.querySelector('#wm-simple-toggle-btn');
    var knob = row.querySelector('#wm-simple-toggle-knob');
    function paint(on){
      btn.setAttribute('aria-checked', on?'true':'false');
      btn.style.background = on ? '#2563eb' : '#334155';
      knob.style.left = on ? '25px' : '3px';
    }
    paint(window.WM_isSimpleMode());
    btn.addEventListener('click', function(){
      var on = window.WM_toggleSimpleMode();
      paint(on);
    });
  }
  ...
  function init(){
    injectCSS();
    Promise.resolve(readPref()).then(function(on){ apply(on); });
    watch();
    // ayar ekranı sonradan oluşabilir → birkaç kez dene + observer
    injectToggleRow();
    try{
      new MutationObserver(function(){ injectToggleRow(); })
        .observe(document.body, {childList:true, subtree:true});
    }catch(e){ setInterval(injectToggleRow, 1500); }
  }
