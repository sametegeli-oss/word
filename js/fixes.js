...
(function(){
  ...
  function stripDup(root){
    try{
      ...
      // 7) Liste adını (.wc-label) görsel kalabalık olmasın diye küçült (kartta kalır)
      var lbl = (scope.querySelector ? scope.querySelector('#wordCard .wc-label') : null)
              || document.querySelector('#wordCard .wc-label');
      if(lbl){ lbl.style.fontSize='13px'; lbl.style.fontWeight='900'; lbl.style.opacity='.85'; }
  ...
  function ensureMoreButton(){
    try{
      ...
      more.textContent = '⋯';
      more.style.cssText = 'padding:10px 12px;background:var(--bg2,#0f1623);border:1.5px solid var(--border,#475569);border-radius:10px;color:var(--accent,#93c5fd);font-size:18px;font-weight:800;cursor:pointer;line-height:1;min-width:44px;';
