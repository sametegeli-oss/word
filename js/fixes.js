...
(function(){
  ...
  /* Aksiyon butonlarını tek satıra diz: liste adı + Öğrenmeye + Gerçek Hayatta */
  function arrangeActionRow(){
    try{
      ...
      if(!row){
        ...
        row.id = 'wmActionRow';
        row.style.cssText = 'display:flex;align-items:center;gap:8px;flex-wrap:wrap;margin:10px 0;';
        // kartın EN BAŞINA koy
        card.insertBefore(row, card.firstChild);
      }
      // liste adı (label) en başa — stilini sadeleştir, satıra al
      if(label && label.parentElement !== row){
        label.style.cssText = 'font-size:13px;font-weight:900;color:var(--text,#e2e8f0);margin:0;white-space:nowrap;';
        row.appendChild(label);
      }
      // Öğrenmeye Başladım butonu
      if(startBtn && startBtn.parentElement !== row){
        startBtn.style.margin = '0';
        row.appendChild(startBtn);
        // sarı barın boş kalan kabuğunu kaldır
        var bar = document.getElementById('wmFixedLearnedBar');
        if(bar && !bar.querySelector('#wmFixedLearnedBtn')) bar.remove();
      }
      // Gerçek Hayatta butonu — Öğrenmeye'nin yanına
      if(realBtn && realBtn.parentElement !== row){
        realBtn.style.margin = '0';
        row.appendChild(realBtn);
      }
      // 📊 Durum toggle butonu — aksiyon satırına al (yukarı taşı)
      var durumBtn = document.querySelector('.wm-status-toggle');
      if(durumBtn && durumBtn.parentElement !== row){
        durumBtn.style.margin = '0';
        row.appendChild(durumBtn);
      }
    }catch(e){}
  }
