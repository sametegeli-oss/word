...
(function(){
  ...
  // Shadow: kendi ekranını aç, mevcut cümleyi seç (sc-shadow → selectShadowPhrase)
  function actShadow(){
    var sent=curSent();
    try{ if(window.openShadowMode) openShadowMode(); }catch(e){}
    // shadowPhrases yüklendikten sonra mevcut cümleye karşılık geleni seç
    var tries=0, iv=setInterval(function(){
      tries++;
      try{
        var arr=window.shadowPhrases;
        if(Array.isArray(arr) && arr.length){
          var t=-1;
          for(var k=0;k<arr.length;k++){ if((arr[k].en||'').trim()===sent.trim()){ t=k; break; } }
          if(t>=0 && window.selectShadowPhrase){ window.selectShadowPhrase(t); clearInterval(iv); return; }
          clearInterval(iv); return; // liste var ama eşleşme yok → ilk cümlede kalsın
        }
      }catch(e){}
