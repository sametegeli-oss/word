...
(function(){
  ...
  function ensureScreen(){
    ...
    document.getElementById('wm-s-prev').addEventListener('click', function(){ try{ window.WM_forcePrevWord ? window.WM_forcePrevWord() : (window.prevWord&&window.prevWord()); }catch(e){} setTimeout(render, 30); });
    document.getElementById('wm-s-next').addEventListener('click', function(){ try{ window.WM_forceNextWord ? window.WM_forceNextWord() : (window.nextWord&&window.nextWord()); }catch(e){} setTimeout(render, 30); });
  ...
  function ensureSheet(){
    ...
    sh.innerHTML='<div class="wm-s-grip"></div><div class="wm-s-ttl">Daha fazla</div><div class="wm-s-grid" id="wm-s-grid"><div class="wm-s-emp">Yakında: telaffuz, gramer, açıklama…</div></div>';
  ...
  // ileride buton eklemek için: WM_addSimpleSheetButton('Telaffuz','🎤',fn)
  window.WM_addSimpleSheetButton = function(label, icon, onClick){
    ensureSheet(); var g=document.getElementById('wm-s-grid'); if(!g) return;
    var emp=g.querySelector('.wm-s-emp'); if(emp) emp.remove();
    var b=document.createElement('button'); b.className='wm-s-b'; b.innerHTML=(icon?icon+' ':'')+label;
  ...
  /* ---- yan menüye düğme ekle (rebuildTools listesine) ---- */
  function injectMenuButton(){
    // rebuildTools fixes.js'te araç panelini kuruyor; oradaki "Kelime Ekranı" düğmesinin
    // yanına bizimkini koymak için paneli DOM'dan bul.
    try{
      // Zaten eklediysek çık
      if(document.getElementById('wm-simple-menu-btn')) return true;
      // "Kelime Ekranı" yazan düğmeyi bul
      var btns = document.querySelectorAll('button, a, .wm-tool, [onclick]');
      var anchor=null;
      for(var k=0;k<btns.length;k++){
        var t=(btns[k].textContent||'').trim();
        if(t.indexOf('Kelime Ekranı')>=0){ anchor=btns[k]; break; }
      }
      if(!anchor) return false;
      var b=anchor.cloneNode(true);
      b.id='wm-simple-menu-btn';
      b.removeAttribute('onclick');
      b.textContent='📖 Sade Kelime';
      b.addEventListener('click', function(e){ e.preventDefault(); ensureScreen(); try{ if(window.showScreen) showScreen('sc-simple'); }catch(e){} render(); });
  ...
  function init(){
    ensureScreen();
    // menü düğmesi: panel sonradan kurulabilir → dene + observer
    if(!injectMenuButton()){
      var tries=0, iv=setInterval(function(){ tries++; if(injectMenuButton()||tries>20) clearInterval(iv); }, 500);
    }
    // sade ekran aktifken ileri/geri klavye/diğer yollarla değişirse yeniden çiz
    try{
      new MutationObserver(function(){
