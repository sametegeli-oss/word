console.log('🗺️ learning-path.js YÜKLENDİ — sürüm v5');
...
(function WMLearningPathUI(){
  ...
  function injectCSS(){
    ...
    /* ════ ANA MENÜ (sc-menu) — DERS EKRANIYLA AYNI KOYU/MİNİMAL STİL ════
       Legacy .btn-blue/.btn-* gradient'lerini ez (background-image:none + !important). */
    #sc-menu > div[style*="grid"]{ gap:10px !important; }
    #sc-menu button{
    ...
    #sc-menu button:active{ transform:scale(.97) !important; }
    /* kart içindeki yazı renklerini sabitle (inline color:#fff'i ez) */
    #sc-menu button div{ color:#e2e8f0 !important; }
    /* başlık ve alt başlık */
    #sc-menu h1{ color:#f1f5f9 !important; -webkit-text-fill-color:#f1f5f9 !important; font-weight:900 !important; }
    #sc-menu p{ color:#94a3b8 !important; }
    /* yol.html butonu menü stiliyle uyumlu kalsın (kendi gradient'i var, korunur) */
    `;
    ...
    function clean(){
      try {
        ...
        document.querySelectorAll('button, a, [onclick]').forEach(function(el){
          ...
          if(t.length>14) return;                          // uzun metin = içerik butonu değil
          // tam olarak "Mining" veya "Ses" (emoji önekiyle) yazan yüzen butonlar
          if(!/^(⛏️?\s*)?Mining$/i.test(t) && !/^(🔊\s*)?Ses$/i.test(t)) return;
