...
(function WMLearningPathUI(){
  ...
  function injectCSS(){
    ...
    #sc-path .wp-actions{ display:grid; grid-template-columns:1fr 1fr; gap:8px; margin-top:14px; }
    #sc-path .wp-act{ padding:10px 12px; border-radius:12px; border:none; font-family:'Nunito',sans-serif;
      font-size:13px; font-weight:800; cursor:pointer; color:#fff; transition:transform .15s, filter .15s;
    ...
    #sc-path .wp-mini-acts{ margin-top:14px; padding:12px; border-radius:14px;
      background:rgba(255,255,255,.03); border:1px solid rgba(255,255,255,.07); }
    #sc-path .wp-mini-title{ font-size:11px; font-weight:800; color:#94a3b8; text-transform:uppercase;
      letter-spacing:1px; margin-bottom:8px; }
    #sc-path .wp-mini-row{ display:grid; grid-template-columns:repeat(4,1fr); gap:6px; }
    #sc-path .wp-mini{ padding:8px 4px; border-radius:10px; border:1px solid rgba(255,255,255,.1);
      background:rgba(255,255,255,.05); color:#cbd5e1; font-family:'Nunito',sans-serif; font-size:11px;
      font-weight:700; cursor:pointer; transition:transform .12s, background .2s; }
    #sc-path .wp-mini:hover{ background:rgba(139,92,246,.15); }
    #sc-path .wp-mini:active{ transform:scale(.95); }
  ...
  function renderLesson(sc){
    ...
    var body = '<div class="wp-lesson">'
      ...
      +'<div class="wp-actions">'
        ...
        +'<button class="wp-act ai full" onclick="WMPath.explain()">🧠 Açıkla (AI)</button>'
        +'<button class="wp-act know full" onclick="WMPath.mark()">'+(known?'✓ Biliyorum':'✅ Biliyorum')+'</button>'
      ...
      +'<div class="wp-mini-acts">'
        ...
        +'</div>'
        +'<button class="wp-mini-gen" onclick="WMPath.genSimilar()">✨ Benzer cümleler üret</button>'
        +'<button class="wp-mini-gen test" onclick="WMPath.aiTest()">📝 Bu bölümle ilgili AI Test</button>'
...
(function WMLearningPathPlus(){
  ...
  P.practiceScenario = function(){
    ...
    // doğru konuşma ekranını aç (openConversationSim / openConversationPartner)
    var opened=false;
    try {
    ...
    setTimeout(function(){
      try {
        ...
        else if (window.showToast){ window.showToast('💬 Konuşma', topic+' konusu hazır'); }
...
(function WMLearningPathReinforce(){
  ...
  P.practiceShadow = function(){
    ...
    // shadow ekranını bu modülün cümleleriyle besle (global shadowPhrases'i ez)
    try {
      window.openShadowMode();        // ekranı açar + loadShadowPhrases (words listesini koyar)
      setTimeout(function(){
        try {
          if(pool.length && typeof window.shadowPhrases!=='undefined'){
            window.shadowPhrases = pool.map(function(it){ return { en:it.en, tr:it.tr||'', word:it.word||'', done:false, score:null }; });
            window.shadowIdx = 0;
            if(typeof window.renderShadowList==='function') window.renderShadowList();
            if(typeof window.selectShadowPhrase==='function') window.selectShadowPhrase(0);
          }
          // seviyeye göre konuşma hızı (A1 yavaş → C ileri hızlı)
          try {
          ...
          if(window.showToast) window.showToast('🎧 Shadowing', lvs.cefr+' · '+pool.length+' cümle · '+lvs.speed+'x');
        } catch(e){}
      }, 320);
  ...
  P.practiceStory = function(){
    ...
    try {
      // seviyeyi modüle göre ayarla (beginner/intermediate/advanced)
      try {
        if(typeof window.setStoryLevel==='function') window.setStoryLevel(lvs.story);
        else window.selectedStoryLevel=lvs.story;
      } catch(e){}
      ...
      setTimeout(function(){
        ...
        // openStoryScreen seviyeyi sıfırlamış olabilir → tekrar uygula
        try { if(typeof window.setStoryLevel==='function') window.setStoryLevel(lvs.story); else window.selectedStoryLevel=lvs.story; } catch(e){}
        if(window.showToast) window.showToast('📖 Hikaye', lvs.cefr+' · '+topic);
      }, 300);
  ...
  P.practicePodcast = function(){
    ...
    try {
      try {
        if(typeof window.setPodcastLevel==='function') window.setPodcastLevel(lvs.podcast);
        else window.selectedPodcastLevel=lvs.podcast;
      } catch(e){}
      window.openPodcastScreen();
      setTimeout(function(){
        var t=document.getElementById('podcastTopic');
        if(t){ if('value' in t) t.value=topic; else t.textContent=topic; }
        try { if(typeof window.setPodcastLevel==='function') window.setPodcastLevel(lvs.podcast); else window.selectedPodcastLevel=lvs.podcast; } catch(e){}
        if(window.showToast) window.showToast('🎙️ Podcast', lvs.cefr+' · '+topic);
      }, 300);
  ...
  function css(){
    ...
    #sc-path .wp-similar-item{ display:flex; align-items:center; gap:8px; padding:8px 10px; margin-bottom:6px;
      border-radius:10px; background:rgba(139,92,246,.08); border:1px solid rgba(139,92,246,.2); }
    #sc-path .wp-similar-item .se{ font-size:13px; font-weight:700; color:#f1f5f9; flex:1; }
    #sc-path .wp-similar-item .st{ font-size:11px; color:#94a3b8; }
    #sc-path .wp-similar-play{ background:rgba(255,255,255,.1); border:none; border-radius:8px;
      width:30px; height:30px; cursor:pointer; font-size:13px; flex:0 0 30px; }`;
    document.head.appendChild(st);
  }
  css();
  if(document.readyState==='loading') document.addEventListener('DOMContentLoaded', css);
