...
(function WMLearningPathUI(){
  ...
  function injectCSS(){
    ...
    /* MODERN MİNİMAL ders butonları: koyu cam zemin, ince çerçeve, sade */
    #sc-path .wp-act{ padding:13px 8px; border-radius:14px; border:1px solid rgba(255,255,255,.09);
      background:rgba(255,255,255,.035); font-family:'Nunito',sans-serif;
      font-size:12.5px; font-weight:700; cursor:pointer; color:#e2e8f0;
      transition:background .18s, border-color .18s, transform .12s; white-space:nowrap;
      display:flex; align-items:center; justify-content:center; gap:6px; }
    #sc-path .wp-act:hover{ background:rgba(255,255,255,.07); border-color:rgba(255,255,255,.18); }
    #sc-path .wp-act:active{ transform:scale(.97); }
    /* renk sadece ikon/ince alt-aksanda — minimal */
    #sc-path .wp-act.listen:hover{ border-color:rgba(59,130,246,.5); }
    #sc-path .wp-act.speak:hover{ border-color:rgba(139,92,246,.5); }
    #sc-path .wp-act.ai:hover{ border-color:rgba(245,158,11,.5); }
    #sc-path .wp-act.know{ background:rgba(34,197,94,.1); border-color:rgba(34,197,94,.3); color:#86efac; }
    #sc-path .wp-act.know:hover{ background:rgba(34,197,94,.18); border-color:rgba(34,197,94,.5); }
    #sc-path .wp-act.full{ grid-column:1 / -1; }
    #sc-path .wp-act.wp-half{ grid-column:span 2; }
    #sc-path .wp-act.ghost{ background:rgba(255,255,255,.02); color:#94a3b8; }
    ...
    #sc-path .wp-mini{ padding:10px 4px; border-radius:11px; border:1px solid rgba(255,255,255,.08);
      background:rgba(255,255,255,.03); color:#cbd5e1; font-family:'Nunito',sans-serif; font-size:11px;
      font-weight:700; cursor:pointer; transition:background .18s, border-color .18s, transform .12s; }
    #sc-path .wp-mini:hover{ background:rgba(255,255,255,.07); border-color:rgba(255,255,255,.16); }
    #sc-path .wp-mini:hover{ background:rgba(139,92,246,.15); }
    #sc-path .wp-mini:active{ transform:scale(.95); }
    #sc-path .wp-mini-wide{ grid-column:span 2; background:rgba(59,130,246,.1); color:#93c5fd; border:1px solid rgba(59,130,246,.25); font-weight:700; }
    #sc-path .wp-mini-wide:hover{ background:rgba(59,130,246,.18); border-color:rgba(59,130,246,.45); }
    #sc-path .wp-mini-wide.test{ background:rgba(245,158,11,.1); color:#fcd34d; border-color:rgba(245,158,11,.25); }
    #sc-path .wp-mini-wide.test:hover{ background:rgba(245,158,11,.18); border-color:rgba(245,158,11,.45); }
  ...
  /* JS TEMİZLEYİCİ: CSS kaçarsa diye, yüzen Mining/Ses/Panel butonlarını
     id'den bağımsız tespit edip gizle (metin eşleşmesiyle). */
  (function hideFloatingFabs(){
    function clean(){
      try {
        // bilinen konteynerler
        ['ffFabs','wmToolsBtn','wmToolsDrawer'].forEach(function(id){
          var el=document.getElementById(id); if(el) el.style.display='none';
        });
        document.querySelectorAll('.ff-fab, .ff-fabs').forEach(function(el){ el.style.display='none'; });
        // metin tabanlı yakalama: sabit konumlu küçük butonlarda Mining/Ses
        document.querySelectorAll('button, a, div').forEach(function(el){
          var t=(el.textContent||'').trim();
          if(t.length<14 && /^[⛏️🎧📊🧠〜~\s]*(Mining|Ses|Panel|Tekrar)\s*$/i.test(t)){
            var cs=window.getComputedStyle(el);
            if(cs.position==='fixed' || (el.parentElement&&window.getComputedStyle(el.parentElement).position==='fixed')){
    ...
    setTimeout(clean,1000); setTimeout(clean,2500);
    // sonradan eklenirlerse yakala
    try{ var mo=new MutationObserver(function(){ clean(); });
      if(document.body) mo.observe(document.body,{childList:true,subtree:true}); }catch(e){}
  })();
    ...
    function toMenu(){
      try {
        ...
        if(menu && word && word.classList.contains('active')){
          ...
          menu.classList.add('active'); menu.style.display='block';
          if(typeof window.showScreen==='function'){ try{ window.showScreen('sc-menu'); }catch(e){} }
        }
      } catch(e){}
    }
    if(document.readyState==='complete') { setTimeout(toMenu,800); }
    else window.addEventListener('load', function(){ setTimeout(toMenu,800); setTimeout(toMenu,1600); });
  })();
