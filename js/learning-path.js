...
(function WMLearningPathUI(){
  ...
  function injectCSS(){
    ...
    #sc-path .wp-scene .wp-scene-grad{ position:absolute; inset:0;
      background:linear-gradient(180deg,rgba(8,11,18,.45) 0%,rgba(8,11,18,.55) 40%,rgba(8,11,18,.95) 100%); }
    #sc-path .wp-scene .wp-scene-body{ position:relative; z-index:2; padding:16px; width:100%;
      background:linear-gradient(180deg,transparent,rgba(8,11,18,.6)); }
    #sc-path .wp-scene .wp-en{ font-size:21px; font-weight:900; color:#fff; line-height:1.35; letter-spacing:-.3px;
      text-shadow:0 2px 8px rgba(0,0,0,.95), 0 1px 2px rgba(0,0,0,1); }
    #sc-path .wp-scene .wp-en .wp-w{ text-shadow:0 2px 8px rgba(0,0,0,.95), 0 1px 3px rgba(0,0,0,1); }
    #sc-path .wp-scene .wp-tr{ font-size:14px; color:#e2e8f0; margin-top:6px; font-weight:600;
    ...
    #sc-path .wp-actions{ display:grid; grid-template-columns:repeat(4,1fr); gap:7px; margin-top:14px; }
    /* Yüzen Mining/Ses/Panel/Tekrar/Shadow ve Araçlar butonlarını TÜM uygulamada gizle */
    #ffFabs, .ff-fabs, #wmToolsBtn, #wmToolsDrawer { display:none !important; visibility:hidden !important; }
    #sc-path .wp-act{ padding:11px 6px; border-radius:12px; border:none; font-family:'Nunito',sans-serif;
      font-size:12px; font-weight:800; cursor:pointer; color:#fff; transition:transform .15s, filter .15s; white-space:nowrap;
      display:flex; align-items:center; justify-content:center; gap:5px; }
    #sc-path .wp-act:active{ transform:scale(.97); }
    @media (max-width:420px){
      #sc-path .wp-actions{ gap:5px; }
      #sc-path .wp-act{ padding:10px 3px; font-size:11px; gap:3px; flex-direction:column; }
      #sc-path .wp-act.wp-half{ flex-direction:row; }
    }
    ...
    #sc-path .wp-mini-wide{ grid-column:span 2; background:linear-gradient(135deg,#3b82f6,#2563eb); color:#fff; border:none; font-weight:800; }
    #sc-path .wp-mini-wide.test{ background:linear-gradient(135deg,#f59e0b,#d97706); }
