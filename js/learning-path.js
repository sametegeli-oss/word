console.log('🗺️ learning-path.js YÜKLENDİ — sürüm v8');
...
(function WMLearningPathUI(){
  ...
  function boot(){
    ...
    // AÇILIŞTA OTOMATİK xlsx YÜKLE (veri yoksa) — Modüller'e girmeyi beklemeden
    try{
      I.buildTree();
      if((!I.PATH.tree || !I.PATH.tree.length) && !window.__WM_BOOT_AUTOLOAD__){
        window.__WM_BOOT_AUTOLOAD__ = true;
        setTimeout(function(){
          if(typeof P.autoLoadGitHub==='function'){
            P.autoLoadGitHub(function(ok){
              if(ok){ try{ I.buildTree(); }catch(e){} }
            });
          }
        }, 1800);  // legacy kendi açılışını bitirsin, sonra çek
      }
    }catch(e){}
  }
  ...
  (function keepOnMenu(){
    ...
    function toMenu(){
      ...
      try {
        ...
        // kullanıcı kendi gezinmeye başladıysa artık karışma
        if(window.__WM_USER_NAV__){ settled=true; return; }
        // kelime ekranı aktifse menüye döndür (legacy showScreen ile)
        if(menu && word && word.classList.contains('active')){
          if(typeof window.showScreen==='function'){ try{ window.showScreen('sc-menu'); }catch(e){} }
        }
      } catch(e){}
    }
    // Açılışta legacy birkaç kez kelime ekranına atabiliyor (xlsx auto-load bitince de).
    // Bu yüzden ilk ~6 saniye boyunca defalarca menüye geri çek.
    [400,900,1500,2200,3000,4200,6000].forEach(function(ms){ setTimeout(toMenu, ms); });
    window.addEventListener('load', function(){ [600,1500,3000,5000].forEach(function(ms){ setTimeout(toMenu, ms); }); });
    // Kullanıcı bir tab/karta tıklarsa __WM_USER_NAV__ işaretle
...
(function moduleGenerator(){
  ...
  function render(){
    ...
    var html = '<div class="mg-wrap">'
      ...
      +'<div class="mg-card">'
        ...
        +'<input id="mgTopic" class="mg-input" placeholder="örn: Present Perfect, Passive Voice, Conditionals..." '
        +'value="'+(state.topic||'').replace(/"/g,'&quot;')+'" oninput="WMModGen.setTopic(this.value)">'
      +'</div>'
      +'<div class="mg-card">'
        +'<div class="mg-row">'
  ...
  function addToPath(){
    ...
    try{
      ...
      // ağacı yeniden kur
      if(window.WMPath && typeof window.WMPath.build==='function'){ try{ window.WMPath.build(); }catch(_){} }
