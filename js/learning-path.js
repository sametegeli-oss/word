...
(function WMLearningPathReinforce(){
  ...
  P.practiceShadow = function(){
    var m=curModule(); var sents=moduleSentences(m,1);
    var first = sents[0] || (curLesson()&&curLesson().items[0]);
    var started=false;
    try { if(typeof window.switchTab==='function'){ window.switchTab('shadow'); started=true; } }catch(e){}
    setTimeout(function(){
      try {
        // hedef cümleyi shadow ekranına yaz (varsa)
        var tgt=document.getElementById('shadowTargetText');
        var tr=document.getElementById('shadowTrText');
        if(tgt&&first){ tgt.textContent=first.en; }
        if(tr&&first){ tr.textContent=first.tr||''; }
        if(typeof window.openShadowMode==='function') window.openShadowMode();
        else if(typeof window.startShadowing==='function') window.startShadowing();
      } catch(e){ if(window.showToast)window.showToast('Shadowing','Ekran açıldı'); }
    }, started?260:0);
  };

  /* ---- Hikaye köprüsü ---- */
  P.practiceStory = function(){
    var m=curModule(); var topic=moduleTopic(m);
    var started=false;
    try { if(typeof window.switchTab==='function'){ window.switchTab('story'); started=true; } }catch(e){}
    setTimeout(function(){
      try {
        var t=document.getElementById('storyTopic')||document.getElementById('storyPrompt');
        if(t){ t.value=topic; }
        if(typeof window.generateStory==='function') window.generateStory();
      } catch(e){ if(window.showToast)window.showToast('Hikaye','Ekran açıldı'); }
    }, started?260:0);
  };
  ...
  P.practicePodcast = function(){
    var m=curModule(); var topic=moduleTopic(m);
    var started=false;
    try { if(typeof window.switchTab==='function'){ window.switchTab('podcast'); started=true; } }catch(e){}
    setTimeout(function(){
      try {
        var t=document.getElementById('podcastTopic');
        if(t){ t.value=topic; }
        if(typeof window.generatePodcast==='function') window.generatePodcast();
      } catch(e){ if(window.showToast)window.showToast('Podcast','Ekran açıldı'); }
    }, started?260:0);
  };
